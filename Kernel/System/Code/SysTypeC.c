...
#define GET_TYPE_C_TOTAL_SRC_PDO_CNT(x)                 (g_pstTypeCStatus[SysTypeCGetTypeCIndex(x)].b4TotalSrcPdoCnt)
#define SET_TYPE_C_TOTAL_SRC_PDO_CNT(x, y)              (g_pstTypeCStatus[SysTypeCGetTypeCIndex(x)].b4TotalSrcPdoCnt = (y))

#define GET_TYPE_C_TOTAL_SNK_PDO_CNT(x)                 (g_pstTypeCStatus[SysTypeCGetTypeCIndex(x)].b4TotalSnkPdoCnt)
#define SET_TYPE_C_TOTAL_SNK_PDO_CNT(x, y)              (g_pstTypeCStatus[SysTypeCGetTypeCIndex(x)].b4TotalSnkPdoCnt = (y))
...
void SysTypeCSetPortControllerPresentPower(BYTE ucInputPort, WORD usPresentPower);
...
BYTE SysTypeCGetTotalSrcPdoCnt(BYTE ucInputPort);
BYTE SysTypeCGetTotalSnkPdoCnt(BYTE ucInputPort);
...
WORD SysTypeCGetAdcOutputVoltage(BYTE ucInputPort);
WORD SysTypeCGetAdcOutputCurrent(BYTE ucInputPort);
...
void SysTypeCCheckSrcPdoChange(BYTE ucInputPort, BYTE ucPdoIndex, StructTypeCUserPDO stTypeCUserPdo)
{
    ...
    else if(stTypeCSrcPdo.enumPdoType == _PD_3_2_SPR_ADJUSTABLE_PDO)
    {
        if(stTypeCSrcPdo.enumPeakCurrent != stTypeCUserPdo.enumPeakCurrent)
        {
            SET_TYPE_C_PDO_CHG_STATUS(ucInputPort);
        ...
        if(stTypeCSrcPdo.usSprAvs15VMaxCurrent != stTypeCUserPdo.usSprAvs15VMaxCurrent)
        {
            SET_TYPE_C_PDO_CHG_STATUS(ucInputPort);
            return;
        }

        if(stTypeCSrcPdo.usSprAvs20VMaxCurrent != stTypeCUserPdo.usSprAvs20VMaxCurrent)
        {
            SET_TYPE_C_PDO_CHG_STATUS(ucInputPort);
            return;
        }
...
EnumPdoCheck SysTypeCSetPortCtrlSourceCap(BYTE ucInputPort)
{
    ...
    BYTE ucPdoIndex = 0;
    WORD usMaxPower = 0;
    StructTypeCUserPDO stTypeCUserPdo = {_PD_FIX_SUPPLY_PDO , _PD_PEAK_CUR_NONE , 0x00 , 0x00 , 0x00 , 0x00 , 0x00};
    ...
    SET_TYPE_C_SPM_OPERATION(UserCommonPortControllerSpmOperation());
    ...
    switch(ucInputPort)
    {
        ...
        case _D0_INPUT_PORT:
            ...
            if(UserCommonPortControllerPdoReference(_D0_INPUT_PORT, _TYPE_C_POWER_SRC) == _TYPE_C_PDO_FROM_USER)
            {
                ...
                SysTypeCSetPortControllerMaxPower(_D0_INPUT_PORT, _D0_DP_TYPE_C_PORT_MAX_POWER);

                // 3. Get Present Power [Unit = 0.1 W]
                // Note: AE is Used to Use "UserInterfaceTypeCGetMaxPower" to Indicate Present Power
                usMaxPower = UserCommonPortControllerGetMaxPower(_D0_INPUT_PORT);
                UserCommonPortControllerGetPresentAvailablePower(_D0_INPUT_PORT, usMaxPower);
                SysTypeCSetPortControllerPresentPower(_D0_INPUT_PORT, usMaxPower);

                // 4. Get Total Number of SPR PDO (Enumeration Range 1 ~ 7) and EPR PDO (Enumeration Range 0 ~ 4)
                UserCommonPortControllerGetPdoCount(_D0_INPUT_PORT, _TYPE_C_POWER_SRC, &ucSPRPdoCount, &ucEPRPdoCount);

                if(ucEPRPdoCount != 0)
                {
                    SET_TYPE_C_TOTAL_SRC_PDO_CNT(_D0_INPUT_PORT, (7 + ucEPRPdoCount));
                }
                else
                {
                    SET_TYPE_C_TOTAL_SRC_PDO_CNT(_D0_INPUT_PORT, ucSPRPdoCount);
                }

                if(GET_TYPE_C_TOTAL_SRC_PDO_CNT(_D0_INPUT_PORT) != SysTypeCGetPortControllerPdoCnt(_D0_INPUT_PORT, _TYPE_C_POWER_SRC))
                {
                ...
                // 5. Get SPR and EPR PDO Info and Write to Port Controller
                for(ucPdoIndex = 1; ucPdoIndex <= GET_TYPE_C_TOTAL_SRC_PDO_CNT(_D0_INPUT_PORT); ucPdoIndex++)
                {
        ...
        case _D1_INPUT_PORT:
            ...
            if(UserCommonPortControllerPdoReference(_D1_INPUT_PORT, _TYPE_C_POWER_SRC) == _TYPE_C_PDO_FROM_USER)
            {
                ...
                // 2. Get Max Power [Unit = 0.1 W]
                SysTypeCSetPortControllerMaxPower(_D1_INPUT_PORT, _D1_DP_TYPE_C_PORT_MAX_POWER);

                // 3. Get Present Power [Unit = 0.1 W]
                // Note: AE is Used to Use "UserInterfaceTypeCGetMaxPower" to Indicate Present Power
                usMaxPower = UserCommonPortControllerGetMaxPower(_D1_INPUT_PORT);
                UserCommonPortControllerGetPresentAvailablePower(_D1_INPUT_PORT, usMaxPower);
                SysTypeCSetPortControllerPresentPower(_D1_INPUT_PORT, usMaxPower);
                ...
                if(ucEPRPdoCount != 0)
                {
                    SET_TYPE_C_TOTAL_SRC_PDO_CNT(_D1_INPUT_PORT, (7 + ucEPRPdoCount));
                }
                else
                {
                    SET_TYPE_C_TOTAL_SRC_PDO_CNT(_D1_INPUT_PORT, ucSPRPdoCount);
                }

                if(GET_TYPE_C_TOTAL_SRC_PDO_CNT(_D1_INPUT_PORT) != SysTypeCGetPortControllerPdoCnt(_D1_INPUT_PORT, _TYPE_C_POWER_SRC))
                {
                ...
                // 5. Get SPR and EPR PDO Info and Write to Port Controller
                for(ucPdoIndex = 1; ucPdoIndex <= GET_TYPE_C_TOTAL_SRC_PDO_CNT(_D1_INPUT_PORT); ucPdoIndex++)
                {
                    // Get User 1st - 11th PDO Info, and Update to Port Controller
        ...
        case _D6_INPUT_PORT:
            ...
            if(UserCommonPortControllerPdoReference(_D6_INPUT_PORT, _TYPE_C_POWER_SRC) == _TYPE_C_PDO_FROM_USER)
            {
                ...
                // 2. Get Max Power [Unit = 0.1 W]
                SysTypeCSetPortControllerMaxPower(_D6_INPUT_PORT, _D6_DP_TYPE_C_PORT_MAX_POWER);

                // 3. Get Present Power [Unit = 0.1 W]
                // Note: AE is Used to Use "UserInterfaceTypeCGetMaxPower" to Indicate Present Power
                usMaxPower = UserCommonPortControllerGetMaxPower(_D6_INPUT_PORT);
                UserCommonPortControllerGetPresentAvailablePower(_D6_INPUT_PORT, usMaxPower);
                SysTypeCSetPortControllerPresentPower(_D6_INPUT_PORT, usMaxPower);
                ...
                if(ucEPRPdoCount != 0)
                {
                    SET_TYPE_C_TOTAL_SRC_PDO_CNT(_D6_INPUT_PORT, (7 + ucEPRPdoCount));
                }
                else
                {
                    SET_TYPE_C_TOTAL_SRC_PDO_CNT(_D6_INPUT_PORT, ucSPRPdoCount);
                }

                if(GET_TYPE_C_TOTAL_SRC_PDO_CNT(_D6_INPUT_PORT) != SysTypeCGetPortControllerPdoCnt(_D6_INPUT_PORT, _TYPE_C_POWER_SRC))
                {
                ...
                // 5. Get SPR and EPR PDO Info and Write to Port Controller
                for(ucPdoIndex = 1; ucPdoIndex <= GET_TYPE_C_TOTAL_SRC_PDO_CNT(_D6_INPUT_PORT); ucPdoIndex++)
                {
                    // Get User 1st - 11th PDO Info, and Update to Port Controller
    ...
    switch(ucInputPort)
    {
        ...
        case _D0_INPUT_PORT:
            ...
            if(UserCommonPortControllerPdoReference(_D0_INPUT_PORT, _TYPE_C_POWER_SNK) == _TYPE_C_PDO_FROM_USER)
            {
                ...
                if(ucEPRPdoCount != 0)
                {
                    SET_TYPE_C_TOTAL_SNK_PDO_CNT(_D0_INPUT_PORT, (7 + ucEPRPdoCount));
                }
                else
                {
                    SET_TYPE_C_TOTAL_SNK_PDO_CNT(_D0_INPUT_PORT, ucSPRPdoCount);
                }

                // 4. Get PDO Info and Write to Port Controller
                for(ucPdoIndex = 1; ucPdoIndex <= GET_TYPE_C_TOTAL_SNK_PDO_CNT(_D0_INPUT_PORT); ucPdoIndex++)
                {
        ...
        case _D1_INPUT_PORT:
            ...
            if(UserCommonPortControllerPdoReference(_D1_INPUT_PORT, _TYPE_C_POWER_SNK) == _TYPE_C_PDO_FROM_USER)
            {
                ...
                if(ucEPRPdoCount != 0)
                {
                    SET_TYPE_C_TOTAL_SNK_PDO_CNT(_D1_INPUT_PORT, (7 + ucEPRPdoCount));
                }
                else
                {
                    SET_TYPE_C_TOTAL_SNK_PDO_CNT(_D1_INPUT_PORT, ucSPRPdoCount);
                }

                // 4. Get PDO Info and Write to Port Controller
                for(ucPdoIndex = 1; ucPdoIndex <= GET_TYPE_C_TOTAL_SNK_PDO_CNT(_D1_INPUT_PORT); ucPdoIndex++)
                {
                    // Get User 1st - 7th PDO Info, and Update to Port Controller
        ...
        case _D6_INPUT_PORT:
            ...
            if(UserCommonPortControllerPdoReference(_D6_INPUT_PORT, _TYPE_C_POWER_SNK) == _TYPE_C_PDO_FROM_USER)
            {
                ...
                if(ucEPRPdoCount != 0)
                {
                    SET_TYPE_C_TOTAL_SNK_PDO_CNT(_D6_INPUT_PORT, (7 + ucEPRPdoCount));
                }
                else
                {
                    SET_TYPE_C_TOTAL_SNK_PDO_CNT(_D6_INPUT_PORT, ucSPRPdoCount);
                }

                // 4. Get PDO Info and Write to Port Controller
                for(ucPdoIndex = 1; ucPdoIndex <= GET_TYPE_C_TOTAL_SNK_PDO_CNT(_D6_INPUT_PORT); ucPdoIndex++)
                {
                    // Get User 1st - 7th PDO Info, and Update to Port Controller
...
#elif((_D0_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_RTS5400_SERIES) && (_TYPE_C_RTS5400_SERIES_PDO_INFO_SUPPORT == _ON))
            UserCommonPortControllerGet5400PdoCnt(_D0_INPUT_PORT, enumPowerRole, &ucSPRPdoCnt, &ucEPRPdoCnt);
...
#elif((_D1_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_RTS5400_SERIES) && (_TYPE_C_RTS5400_SERIES_PDO_INFO_SUPPORT == _ON))
            UserCommonPortControllerGet5400PdoCnt(_D1_INPUT_PORT, enumPowerRole, &ucSPRPdoCnt, &ucEPRPdoCnt);
...
#elif((_D6_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_RTS5400_SERIES) && (_TYPE_C_RTS5400_SERIES_PDO_INFO_SUPPORT == _ON))
            UserCommonPortControllerGet5400PdoCnt(_D6_INPUT_PORT, enumPowerRole, &ucSPRPdoCnt, &ucEPRPdoCnt);
...
//--------------------------------------------------
// Description  : Set User-Defined Source Present Power to PD Port Ctrl
// Input Value  : ucInputPort --> _D0_INPUT_PORT / _D1_INPUT_PORT..., usPresentPower
// Output Value : None
//--------------------------------------------------
void SysTypeCSetPortControllerPresentPower(BYTE ucInputPort, WORD usPresentPower)
{
    ucInputPort = ucInputPort;
    usPresentPower = usPresentPower;
...
#if(_D0_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_EMBEDDED)
            if(ScalerTypeCGetPresentPower(_D0_INPUT_PORT) != usPresentPower)
            {
                SET_TYPE_C_PDO_CHG_STATUS(_D0_INPUT_PORT);
                ScalerTypeCSetPresentPower(_D0_INPUT_PORT, usPresentPower);
            }
#elif((_D0_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_RTS5400_SERIES) && (_TYPE_C_RTS5400_SERIES_PDO_INFO_SUPPORT == _ON))
#if(_PORT_CONTROLLER_PD3_1_EPR_SUPPORT == _ON)
            if(UserCommonPortControllerGet5400MaxPower(_D0_INPUT_PORT) != usPresentPower)
            {
                SET_TYPE_C_PDO_CHG_STATUS(_D0_INPUT_PORT);
                UserCommonPortControllerSet5400MaxPower(_D0_INPUT_PORT, usPresentPower);
            }
#endif  // End of #if(_PORT_CONTROLLER_PD3_1_EPR_SUPPORT == _ON)
#elif(_D0_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_USER)
            // UserCommonPortControllerSetUserPresentPower(_D0_INPUT_PORT, usPresentPower);
#endif
            break;
#endif  // End of #if(_D0_DP_TYPE_C_PORT_CTRL_SUPPORT == _ON)
...
#if(_D1_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_EMBEDDED)
            if(ScalerTypeCGetPresentPower(_D1_INPUT_PORT) != usPresentPower)
            {
                SET_TYPE_C_PDO_CHG_STATUS(_D1_INPUT_PORT);
                ScalerTypeCSetPresentPower(_D1_INPUT_PORT, usPresentPower);
            }
#elif((_D1_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_RTS5400_SERIES) && (_TYPE_C_RTS5400_SERIES_PDO_INFO_SUPPORT == _ON))
#if(_PORT_CONTROLLER_PD3_1_EPR_SUPPORT == _ON)
            if(UserCommonPortControllerGet5400MaxPower(_D1_INPUT_PORT) != usPresentPower)
            {
                SET_TYPE_C_PDO_CHG_STATUS(_D1_INPUT_PORT);
                UserCommonPortControllerSet5400MaxPower(_D1_INPUT_PORT, usPresentPower);
            }
#endif  // End of #if(_PORT_CONTROLLER_PD3_1_EPR_SUPPORT == _ON)
#elif(_D1_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_USER)
            // UserCommonPortControllerSetUserPresentPower(_D1_INPUT_PORT, usPresentPower);
#endif
            break;
#endif  // End of #if(_D1_DP_TYPE_C_PORT_CTRL_SUPPORT == _ON)
...
#if(_D6_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_EMBEDDED)
            if(ScalerTypeCGetPresentPower(_D6_INPUT_PORT) != usPresentPower)
            {
                SET_TYPE_C_PDO_CHG_STATUS(_D6_INPUT_PORT);
                ScalerTypeCSetPresentPower(_D6_INPUT_PORT, usPresentPower);
            }
#elif((_D6_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_RTS5400_SERIES) && (_TYPE_C_RTS5400_SERIES_PDO_INFO_SUPPORT == _ON))
#if(_PORT_CONTROLLER_PD3_1_EPR_SUPPORT == _ON)
            if(UserCommonPortControllerGet5400MaxPower(_D6_INPUT_PORT) != usPresentPower)
            {
                SET_TYPE_C_PDO_CHG_STATUS(_D6_INPUT_PORT);
                UserCommonPortControllerSet5400MaxPower(_D6_INPUT_PORT, usPresentPower);
            }
#endif  // End of #if(_PORT_CONTROLLER_PD3_1_EPR_SUPPORT == _ON)
#elif(_D6_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_USER)
            // UserCommonPortControllerSetUserPresentPower(_D6_INPUT_PORT, usPresentPower);
#endif
            break;
#endif  // End of #if(_D6_DP_TYPE_C_PORT_CTRL_SUPPORT == _ON)
...
#elif((_D0_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_RTS5400_SERIES) && (_TYPE_C_RTS5400_SERIES_PDO_INFO_SUPPORT == _ON))
            UserCommonPortControllerSet5400PdoCnt(_D0_INPUT_PORT, enumPowerRole, ucSPRPdoCnt, ucEPRPdoCnt);
#elif(_D0_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_USER)
...
#elif((_D1_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_RTS5400_SERIES) && (_TYPE_C_RTS5400_SERIES_PDO_INFO_SUPPORT == _ON))
            UserCommonPortControllerSet5400PdoCnt(_D1_INPUT_PORT, enumPowerRole, ucSPRPdoCnt, ucEPRPdoCnt);
#elif(_D1_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_USER)
...
#elif((_D6_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_RTS5400_SERIES) && (_TYPE_C_RTS5400_SERIES_PDO_INFO_SUPPORT == _ON))
            UserCommonPortControllerSet5400PdoCnt(_D6_INPUT_PORT, enumPowerRole, ucSPRPdoCnt, ucEPRPdoCnt);
#elif(_D6_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_USER)
...
void SysTypeCSetPortControllerPdo(BYTE ucInputPort, EnumTypeCPowerRole enumPowerRole, BYTE ucPdoIndex, StructTypeCUserPDO stTypeCUserPdo)
{
    ...
    switch(ucInputPort)
    {
        ...
        case _D0_INPUT_PORT:
            ...
            else if(stTypeCUserPdo.enumPdoType == _PD_3_2_SPR_ADJUSTABLE_PDO)
            {
                ScalerTypeCSetSPRAvsPdo(_D0_INPUT_PORT, enumPowerRole, ucPdoIndex, stTypeCUserPdo.usSprAvs15VMaxCurrent, stTypeCUserPdo.usSprAvs20VMaxCurrent, stTypeCUserPdo.enumPeakCurrent);
            }
        ...
        case _D1_INPUT_PORT:
            ...
            {
                ScalerTypeCSetSPRAvsPdo(_D1_INPUT_PORT, enumPowerRole, ucPdoIndex, stTypeCUserPdo.usSprAvs15VMaxCurrent, stTypeCUserPdo.usSprAvs20VMaxCurrent, stTypeCUserPdo.enumPeakCurrent);
            }
        ...
        case _D6_INPUT_PORT:
            ...
            {
                ScalerTypeCSetSPRAvsPdo(_D6_INPUT_PORT, enumPowerRole, ucPdoIndex, stTypeCUserPdo.usSprAvs15VMaxCurrent, stTypeCUserPdo.usSprAvs20VMaxCurrent, stTypeCUserPdo.enumPeakCurrent);
            }
...
//--------------------------------------------------
// Description  : Return Type C Total Source PDO Count
// Input Value  : ucInputPort
// Output Value : Total Source PDO Count
//--------------------------------------------------
BYTE SysTypeCGetTotalSrcPdoCnt(BYTE ucInputPort)
{
    BYTE ucTotalSrcPdoCnt = 0x00;
    ...
    switch(ucInputPort)
    {
        ...
        case _D0_INPUT_PORT:
            ...
            ucTotalSrcPdoCnt = GET_TYPE_C_TOTAL_SRC_PDO_CNT(_D0_INPUT_PORT);
        ...
        case _D1_INPUT_PORT:
            ...
            ucTotalSrcPdoCnt = GET_TYPE_C_TOTAL_SRC_PDO_CNT(_D1_INPUT_PORT);
        ...
        case _D6_INPUT_PORT:
            ...
            ucTotalSrcPdoCnt = GET_TYPE_C_TOTAL_SRC_PDO_CNT(_D6_INPUT_PORT);
    ...
    return ucTotalSrcPdoCnt;
}

//--------------------------------------------------
// Description  : Return Type C Total Sink PDO Count
// Input Value  : ucInputPort
// Output Value : Total Sink PDO Count
//--------------------------------------------------
BYTE SysTypeCGetTotalSnkPdoCnt(BYTE ucInputPort)
{
    BYTE ucTotalSnkPdoCnt = 0x00;
    ...
    switch(ucInputPort)
    {
        ...
        case _D0_INPUT_PORT:
            ...
            ucTotalSnkPdoCnt = GET_TYPE_C_TOTAL_SNK_PDO_CNT(_D0_INPUT_PORT);
        ...
        case _D1_INPUT_PORT:
            ...
            ucTotalSnkPdoCnt = GET_TYPE_C_TOTAL_SNK_PDO_CNT(_D1_INPUT_PORT);
        ...
        case _D6_INPUT_PORT:
            ...
            ucTotalSnkPdoCnt = GET_TYPE_C_TOTAL_SNK_PDO_CNT(_D6_INPUT_PORT);
...
#elif((_D0_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_RTS5400_SERIES) && (_TYPE_C_RTS5400_SERIES_PDO_INFO_SUPPORT == _ON))
            UserCommonPortControllerGet5400PartnerPdoCnt(_D0_INPUT_PORT, enumPowerRole, &ucSPRPdoCnt, &ucEPRPdoCnt);
...
#elif((_D1_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_RTS5400_SERIES) && (_TYPE_C_RTS5400_SERIES_PDO_INFO_SUPPORT == _ON))
            UserCommonPortControllerGet5400PartnerPdoCnt(_D1_INPUT_PORT, enumPowerRole, &ucSPRPdoCnt, &ucEPRPdoCnt);
...
#elif((_D6_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_RTS5400_SERIES) && (_TYPE_C_RTS5400_SERIES_PDO_INFO_SUPPORT == _ON))
            UserCommonPortControllerGet5400PartnerPdoCnt(_D6_INPUT_PORT, enumPowerRole, &ucSPRPdoCnt, &ucEPRPdoCnt);
...
//--------------------------------------------------
// Description  : Return TypeC ADC Channel Output Voltage Value
// Input Value  : ucInputPort
// Output Value : Output Voltage Value (Unit: 10mV)
//--------------------------------------------------
WORD SysTypeCGetAdcOutputVoltage(BYTE ucInputPort)
{
    WORD usAdcOutputVoltage = 0;
    ucInputPort = ucInputPort;
...
#if(_D0_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_EMBEDDED)
            usAdcOutputVoltage = ScalerTypeCPowerGetAdcOutputVoltage(_D0_INPUT_PORT);
#elif(_D0_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_RTS5400_SERIES)
            // usAdcOutputVoltage = UserCommonPortControllerGet5400AdcOutputVoltage(_D0_INPUT_PORT);
#elif(_D0_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_USER)
            // usAdcOutputVoltage = UserCommonPortControllerGetUserAdcOutputVoltage(_D0_INPUT_PORT);
#endif
...
#if(_D1_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_EMBEDDED)
            usAdcOutputVoltage = ScalerTypeCPowerGetAdcOutputVoltage(_D1_INPUT_PORT);
#elif(_D1_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_RTS5400_SERIES)
            // usAdcOutputVoltage = UserCommonPortControllerGet5400AdcOutputVoltage(_D1_INPUT_PORT);
#elif(_D1_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_USER)
            // usAdcOutputVoltage = UserCommonPortControllerGetUserAdcOutputVoltage(_D1_INPUT_PORT);
#endif
...
#if(_D6_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_EMBEDDED)
            usAdcOutputVoltage = ScalerTypeCPowerGetAdcOutputVoltage(_D6_INPUT_PORT);
#elif(_D6_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_RTS5400_SERIES)
            // usAdcOutputVoltage = UserCommonPortControllerGet5400AdcOutputVoltage(_D6_INPUT_PORT);
#elif(_D6_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_USER)
            // usAdcOutputVoltage = UserCommonPortControllerGetUserAdcOutputVoltage(_D6_INPUT_PORT);
#endif
    ...
    return usAdcOutputVoltage;
}

//--------------------------------------------------
// Description  : Return TypeC ADC Channel Output Current Value
// Input Value  : ucInputPort
// Output Value : Output Current Value (Unit: 10mA)
//--------------------------------------------------
WORD SysTypeCGetAdcOutputCurrent(BYTE ucInputPort)
{
    WORD usAdcOutputCurrent = 0;
    ucInputPort = ucInputPort;
...
#if(_D0_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_EMBEDDED)
            usAdcOutputCurrent = ScalerTypeCPowerGetAdcOutputCurrent(_D0_INPUT_PORT);
#elif(_D0_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_RTS5400_SERIES)
            // usAdcOutputCurrent = UserCommonPortControllerGet5400AdcOutputCurrent(_D0_INPUT_PORT);
#elif(_D0_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_USER)
            // usAdcOutputCurrent = UserCommonPortControllerGetUserAdcOutputCurrent(_D0_INPUT_PORT);
#endif
...
#if(_D1_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_EMBEDDED)
            usAdcOutputCurrent = ScalerTypeCPowerGetAdcOutputCurrent(_D1_INPUT_PORT);
#elif(_D1_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_RTS5400_SERIES)
            // usAdcOutputCurrent = UserCommonPortControllerGet5400AdcOutputCurrent(_D1_INPUT_PORT);
#elif(_D1_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_USER)
            // usAdcOutputCurrent = UserCommonPortControllerGetUserAdcOutputCurrent(_D1_INPUT_PORT);
#endif
...
#if(_D6_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_EMBEDDED)
            usAdcOutputCurrent = ScalerTypeCPowerGetAdcOutputCurrent(_D6_INPUT_PORT);
#elif(_D6_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_RTS5400_SERIES)
            // usAdcOutputCurrent = UserCommonPortControllerGet5400AdcOutputCurrent(_D6_INPUT_PORT);
#elif(_D6_DP_TYPE_C_PORT_CTRL_TYPE == _PORT_CONTROLLER_USER)
            // usAdcOutputCurrent = UserCommonPortControllerGetUserAdcOutputCurrent(_D6_INPUT_PORT);
#endif
