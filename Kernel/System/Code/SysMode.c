    ...
    switch(GET_MODE_STATE())
    {
        ...
        case _MODE_STATUS_ACTIVE:
            ...
            if(GET_MODE_STATE_CHANGE_MODE() == _TRUE)
            {
                ...
                if(ScalerTMDSGetFirstActiveHdcp14BchErrorCheck(SysSourceGetInputPort()) == _TRUE)
#endif
#endif
                {
...
void SysModeSetupIDomainIniValue(void)
{
    ...
    {
        DebugMessageScaler("Update VGA Measure Measure Info", 0);
        DebugMessageScaler("Input Timing HTotal", GET_INPUT_TIMING_HTOTAL());
        DebugMessageScaler("Input Timing HWidth", GET_INPUT_TIMING_HWIDTH());
    }
#endif
...
#if((_DRR_SUPPORT == _ON) && (_FW_DRR_SUPPORT_GEN_TYPE == _DRR_SUPPORT_GEN_II))
...
#if(_FW_DRR_SUPPORT_GEN_TYPE == _DRR_SUPPORT_GEN_II)
...
WORD SysModeDrrGetIVFreq(EnumSourceSearchPort enumSearchPort)
{
    ...
    if(ScalerDrrGetType() != _DRR_NONE_TYPE)
    {
        ...
        {
            SET_DRR_VFREQ_MAX(g_usFREESYNCVFreq_max);
        }
