...
void SysAudioMuteProc(bit bMute)
{
    if(bMute == _ON)
    {
        if(ScalerAudioDVCGetVolumeMuteStatus() == _OFF)
        {
            ...
            if(UserCommonAudioGetAdjustVolumeThroughDVC() == _FALSE)
            {
                ...
                // Audio DVC Hold by Un-Mute
                ScalerAudioDVCAdjustVolume(0x3FFF);
#else
                // Audio DVC Hold by Mute
...
#endif
            {
                // Digital Audio Output DVC Mute
                ScalerAudioMuteProc(_ON, UserInterfaceGetAudioVolume());
            }

            // Set DVC State: Mute State
    ...
    else
    {
        ...
        if(bResult == _TRUE)
        {
            ...
            // Ext. Audio Amplifier Un-Mute
            UserInterfaceAdjustAudioAmpMuteProc(_OFF);

            // Set DVC State: Un-Mute State
