...
WORD ScalerFRCSetOneFrameDVTotal(void)
{
    ...
    {
        usDHF = (DWORD)GET_OUTPUT_VFREQ() * _PANEL_DV_TOTAL / 1000;
    }
    else
    {
        usDHF = (DWORD)GET_OUTPUT_VFREQ() * (DWORD)GET_MDOMAIN_OUTPUT_VHEIGHT() * 1066 / 1000 / 1000;
    }

    usIHF = (DWORD)GET_MDOMAIN_INPUT_VFREQ() * GET_MDOMAIN_INPUT_VTOTAL() / 1000;
    ulFrameDiff = (DWORD)1000 * ((DWORD)1000 * GET_MDOMAIN_INPUT_VFREQ() - (DWORD)1000 * GET_OUTPUT_VFREQ()) / ((DWORD)GET_MDOMAIN_INPUT_VFREQ() * GET_OUTPUT_VFREQ());
    ...
    {
        usDVTotal = (DWORD)1000 * GET_MDOMAIN_OUTPUT_VHEIGHT() / ((ulFrameDiff * 100 / 125) + ((DWORD)1000 * GET_MDOMAIN_INPUT_VHEIGHT() / usIHF)) * 1000 / GET_OUTPUT_VFREQ();
    }
...
BYTE ScalerFRCGetMemRateCtrlParamIndex(StructFRCRateCtrlInfo *pstRateCtrlInfo)
{
    ...
    for(ucIndex = 0; ucIndex < ucCnt; ucIndex++)
    {
        ...
        if(GET_FRC_RCP_HSD_EN(ucIndex) == _FALSE)
        {
            ...
            BYTE ucFrameNum = (GET_MEMORY_FRAME_BUFFER() == _MEMORY_1_FRAME_BUFFER) ? 1 : GET_MEMORY_FRAME_BUFFER();
