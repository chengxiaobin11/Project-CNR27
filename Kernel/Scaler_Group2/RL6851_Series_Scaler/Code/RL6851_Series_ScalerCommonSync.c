    ...
    if(ScalerSyncHDR10CheckPortStatus(enumSourceSearchPort) == _TRUE)
    {
        ...
        switch(enumSourceSearchPort)
        {
            ...
            case _D0_INPUT_PORT:
                ...
                GET_TMDSRX0_HDR10_DATA(pucHDRInfoData);
...
#if(_DP_HDR10_SUPPORT == _ON)
#if(_DP_HDR_VENDOR_SUPPORT != _DP_HDR_VENDOR_SUPPORT_NONE)
            if(ScalerSyncDpHdrCapabilitySwitchProc(GET_DP_EMBEDDED_SWITCH_SELECT()) == _TRUE)
            {
                bDPRx0DoLongHotPlug = _TRUE;
            }
#endif
...
void ScalerSyncHdcp14KeySramGatedClk(bit bEnable)
{
    if(bEnable == _ENABLE)
    {
        ...
        ScalerSetBit(P2_C2_HDCP_PCR, ~(_BIT7 | _BIT4), 0x00);
    }
    else
    {
        // Disable SRAM Module Clock Gating
        ScalerSetBit(P2_C2_HDCP_PCR, ~(_BIT7 | _BIT4), _BIT7);
    }
}
...
void ScalerSyncTMDSRx0HdmiVideoSetting(void)
{
    ...
    else    // No Info Frame Received
    {
        ...
        if(GET_TMDS_RX0_CLK_MODE() == _TMDS_MHL_PPMODE_PHY_SETTING)
        {
            ...
            ScalerSetDataPortBit(P2_C9_HDMI_AP, _P2_CA_PT_51_PORT_PAGE2_HDMI_ACRCR, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), _BIT3);
        }
        else
#endif
