...
#if(_HDMI_HDCP14_BCH_ERROR_CHECK == _ON)
volatile EnumHdmiHdcp14BchChkState g_enumTMDSHdcp14BchChkState;
#endif
...
#if(_HDMI_HDCP14_BCH_ERROR_CHECK == _ON)
bit ScalerTMDSStableDetectHdcp14BchErrorCheck(EnumHdmiHdcp14BchChkState enumBchChkState);
void ScalerTMDSHdcp14BchErrorCheckEvent(EnumScalerTimerEventID enumEventID);
bit ScalerTMDSGetFirstActiveHdcp14BchErrorCheck(BYTE ucInputPort);
void ScalerTMDSHdcp14BchErrorCheckReset(BYTE ucInputPort);
#endif
...
void ScalerTMDSInterruptInitial(BYTE ucInputPort)
{
    ...
    ScalerSetBit(P2_E6_HDMI_FREQDET_CTRL, ~(_BIT7 | _BIT5 | _BIT4 | _BIT2), 0x00);
    ...
    ScalerSetBit(P2_E6_HDMI_FREQDET_CTRL, ~(_BIT2 | _BIT1), 0x00);
...
void ScalerTMDSSetPhy_EXINT0(void) using 1
{
    ...
    else if((GET_INPUT_PIXEL_CLK() >= 1500) && (GET_INPUT_PIXEL_CLK() < 3400))
#else
    else if((GET_INPUT_PIXEL_CLK() >= 1500) && (GET_INPUT_PIXEL_CLK() < 3030))
#endif
    {
        // Analog CDR Auto FLD Counter
        // ref_conuter = 250
    ...
    {
        // Analog CDR Auto FLD Counter = 0
        ScalerSetBit_EXINT(PB_D7_ANA_CDR_08, ~(_BIT7 | _BIT6), 0x00);
        ScalerSetByte_EXINT(PB_D8_ANA_CDR_09, 0x00);
        ScalerSetByte_EXINT(PB_D9_ANA_CDR_10, 0x00);

        // [2:0] rate_sel = 0x00
        ScalerSetBit_EXINT(PB_DC_TMDS_CONT_1, ~(_BIT2 | _BIT1 | _BIT0), 0x00);
        ...
        // [7:0] N_CODE = 0
        ScalerSetByte_EXINT(PB_C3_CMU_01, 0x00);
    }
    ...
    ScalerSetBit_EXINT(PB_AB_SDM_04, ~(_BIT5 | _BIT1 | _BIT0), _BIT1);
    ...
    ScalerSetBit_EXINT(PB_AB_SDM_04, ~(_BIT5 | _BIT0), _BIT0);
    ...
    ScalerSetBit_EXINT(P2_B0_TMDS_CHANNEL_FIFO, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), _BIT6);

    // HDMI 3 Channel FIFO Global Enable
    ScalerSetByte_EXINT(P2_B0_TMDS_CHANNEL_FIFO, _BIT7);
...
void ScalerTMDSIntHandler_EXINT0(void) using 1
{
    ...
    if((ScalerGetByte_EXINT(P2_E6_HDMI_FREQDET_CTRL) & 0x06) == 0x06)
    {
           ...
           (abs(GET_INPUT_PIXEL_CLK() - ((GET_TMDS_OFFLINE_MEASURE_CLK() << 3) / ((GET_COLOR_SPACE() == _COLOR_SPACE_YCBCR422) ? _COLOR_DEPTH_8_BITS : GET_COLOR_DEPTH()))) > usErrorThreshold_2))
        {
            ...
            // Disable Freq. Unstable IRQ
            ScalerSetBit_EXINT(P2_E6_HDMI_FREQDET_CTRL, ~(_BIT2 | _BIT1), 0x00);
            ...
            ScalerSetBit_EXINT(P2_E6_HDMI_FREQDET_CTRL, ~(_BIT7 | _BIT2), 0x00);
            ScalerSetBit_EXINT(P2_E6_HDMI_FREQDET_CTRL, ~(_BIT7 | _BIT2), _BIT7);
...
#if(_HDMI_HDCP14_BCH_ERROR_CHECK == _ON)
        // Unlock Ri'
        ScalerSetByte_EXINT(P2_C3_HDCP_AP, _P2_C4_PT_C1_PORT_PAGE2_HDCP_SIR);
        ScalerSetBit_EXINT(P2_C4_HDCP_DP, ~_BIT4, 0x00);

        // Clear TimerEvenet
        ScalerTimerCancelTimerEvent_EXINT0(_SCALER_TIMER_EVENT_RX0_TMDS_HDCP14_BCH_ERR_CHK);

        SET_TMDS_HDCP14_BCH_CHK_STATE(_HDMI_HDCP14_BCH_AUTH_START);
#endif
...
void ScalerTMDSHdmiAVMuteProc(void)
{
    ...
    {
        // Clear AVMute Flag
        ScalerSetBit(P2_CB_HDMI_SR, ~(_BIT7 | _BIT5 | _BIT2 | _BIT1), _BIT7);
...
void ScalerTMDSMacDdcSwitchCtrl(BYTE ucInputPort)
{
    switch(ucInputPort)
    {
        ...
        case _D0_INPUT_PORT:
            ...
            ScalerSetBit(P2_C2_HDCP_PCR, ~(_BIT4 | _BIT2 | _BIT1), 0x00);
#elif(_D0_DDC_CHANNEL_SEL == _DDC1)
            ScalerSetBit(P2_C2_HDCP_PCR, ~(_BIT4 | _BIT2 | _BIT1), _BIT1);
#endif
            break;
...
{
#if(_HDMI_HDCP14_BCH_ERROR_CHECK == _ON)
    BYTE pucHDCP14FrmCnt[2] = {0};
    BYTE ucCheckTime = 10;    // ms, must > 0
    WORD usRemainTime = 0x00;  // ms
#endif
...
#if(_HDMI_HDCP14_BCH_ERROR_CHECK == _ON)
    // Wait more time for HDCP1.4 Aksv IRQ
    usRemainTime = 80;
    while(GET_TMDS_HDCP14_BCH_CHK_STATE() == _HDMI_HDCP14_BCH_NONE)
    {
        ScalerTimerDelayXms(ucCheckTime);
        usRemainTime -= ucCheckTime;
        if((GET_TMDS_HDCP14_BCH_CHK_STATE() == _HDMI_HDCP14_BCH_AUTH_START) || (usRemainTime < ucCheckTime))
        {
            break;
        }
    }

    // Check BCH Error for HDCP1.4 in _SCALER_TIMER_EVENT_RX0_TMDS_HDCP14_BCH_ERR_CHK window
    switch(GET_TMDS_HDCP14_BCH_CHK_STATE())
    {
        case _HDMI_HDCP14_BCH_AUTH_START:
        case _HDMI_HDCP14_BCH_CHK_WINDOW_START:

            if(GET_TMDS_HDCP14_BCH_CHK_STATE() == _HDMI_HDCP14_BCH_CHK_WINDOW_START)
            {
                usRemainTime = ScalerTimerGetEventRemainTime(_SCALER_TIMER_EVENT_RX0_TMDS_HDCP14_BCH_ERR_CHK);
            }
            else
            {
                usRemainTime = 1000;
            ...
            usRemainTime = (usRemainTime + (ucCheckTime - 1)) / ucCheckTime * ucCheckTime;  // Ceil(usRemainTime, ucCheckTime)
            usRemainTime = MAXOF(usRemainTime, ucCheckTime);

            // Polling HDCP1.4 Frame Counter to wait Encrypt Data
            ScalerGetDataPortByte(P2_C3_HDCP_AP, _P2_C4_PT_C0_PORT_PAGE2_HDCP_FCR, 1, &pucHDCP14FrmCnt[0], _NON_AUTOINC);
            while(usRemainTime >= ucCheckTime)
            {
                ScalerTimerDelayXms(ucCheckTime);
                ScalerGetDataPortByte(P2_C3_HDCP_AP, _P2_C4_PT_C0_PORT_PAGE2_HDCP_FCR, 1, &pucHDCP14FrmCnt[1], _NON_AUTOINC);

                if(pucHDCP14FrmCnt[0] != pucHDCP14FrmCnt[1])
                {
                    break;
                }
            ...
            // Clr BCH Error Flag
            ScalerSetDataPortBit(P2_C9_HDMI_AP, _P2_CA_PT_02_PORT_PAGE2_HDMI_BCHCR, ~_BIT2, 0x00);

            ScalerTimerDelayXms(45);

            // bch check
            if(ScalerGetDataPortBit(P2_C9_HDMI_AP, _P2_CA_PT_02_PORT_PAGE2_HDMI_BCHCR, _BIT2) == _BIT2)
            {
                // Set Display M1 output force to background color and free run mode
                ScalerSetBit(P0_28_VIDEO_DISPLAY_CONTROL_REG, ~(_BIT5 | _BIT3), _BIT5);

                // Disable RGB/clock Output to avoid garbage
                ScalerSetBit(P2_A6_TMDS_OUTCTL, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3), 0x00);

                SET_TMDS_HDCP14_BCH_CHK_STATE(_HDMI_HDCP14_BCH_1ST_ACT_ENC_CHK_FAIL);
                DebugMessageRx0("FirstActive HDCP14 BCH Error Detected", 0x00);
            }
            else
            {
                SET_TMDS_HDCP14_BCH_CHK_STATE(_HDMI_HDCP14_BCH_1ST_ACT_ENC_CHK_PASS);
            }
...
#endif // End of #if(_HDMI_HDCP14_BCH_ERROR_CHECK == _ON)
}
...
bit ScalerTMDSPSPreDetect(BYTE ucInputPort)
{
    ...
    else
    {
        ...
        ScalerSetBit(P2_E6_HDMI_FREQDET_CTRL, ~(_BIT7 | _BIT2), 0x00);
...
BYTE ScalerTMDSScanInputPort(BYTE ucInputPort, BYTE ucInputType)
{
    ...
    if(GET_TMDS_PHY_SET() == _TRUE)
    {
        ...
        ScalerSetByte(P2_CC_HDMI_GPVS, (_BIT6 | _BIT5));
...
#if(_HDMI_HDR10_SUPPORT == _ON)
        // Clear HDR infoframe
        ScalerSetDataPortBit(P2_C9_HDMI_AP, _P2_CA_PT_00_PORT_PAGE2_HDMI_SCR, ~(_BIT7 | _BIT6), _BIT6);
#endif
        ...
        if(ucTMDSType == _SOURCE_HDMI)
        {
            ...
            BYTE ucBchCheck = 0x00;

            // bch check
            for(ucBchCheck = 0; ucBchCheck < 9; ucBchCheck++)
            {
                if(ScalerGetDataPortBit(P2_C9_HDMI_AP, _P2_CA_PT_02_PORT_PAGE2_HDMI_BCHCR, _BIT2) == _BIT2)
                {
                    // Clr BCH Error Flag
            ...
            if(ucBchErrorCount >= 6)
            {
                // Lock Ri' to let next Ri check fail -> Force Source to Re-Auth
                ScalerSetDataPortBit(P2_C3_HDCP_AP, _P2_C4_PT_C1_PORT_PAGE2_HDCP_SIR, ~_BIT4, _BIT4);

                DebugMessageRx0("BCH Error Detected", ucBchErrorCount);
...
#endif
            {
                ...
                if(GET_TMDS_HDMI21_VTEM_RECEIVED() == _TRUE)
                {
                    ...
                    BYTE ucAVIInfoVic = 0x00;

                    // Get Base Frame Rate from SRC
                    if((GET_HDMI_RX0_AVI_INFO_FRAME_RECEIVED() == _TRUE) && (ScalerTMDSRx0HdmiGetAVIInfo(_AVI_INFO_VIC) != 0x00))
                    {
                        ucAVIInfoVic = ((ScalerTMDSRx0HdmiGetAVIInfo(_AVI_INFO_VIC) != 0x00) ? (ScalerTMDSRx0HdmiGetAVIInfo(_AVI_INFO_VIC) - 1) : 0x00);

                        usSourceBrr = ((WORD)ScalerTMDSGetVicVfreqParameter(ucAVIInfoVic) * 10);
                        DebugMessageRx0("VRR BRR (AVI IF)", usSourceBrr);
                    }
                    else if((GET_HDMI_RX0_AVI_INFO_FRAME_RECEIVED() == _TRUE) && (ScalerTMDSRx0HdmiRsv0GetAVIInfo(_AVI_INFO_FR) != 0x00))
                    {
                        usSourceBrr = (ScalerTMDSGetOvtVfreqParameter(ScalerTMDSRx0HdmiRsv0GetAVIInfo(_AVI_INFO_FR)) * 10);
                        DebugMessageRx0("VRR BRR (AVI IF OVT)", usSourceBrr);
                    }
...
bit ScalerTMDSStableDetect(BYTE ucInputPort, BYTE ucSourceType)
{
    ...
    {
        // Check BCH Error caused by Source HDCP1.4 signal
        if(ScalerTMDSStableDetectHdcp14BchErrorCheck(GET_TMDS_HDCP14_BCH_CHK_STATE()) == _FALSE)
        {
            DebugMessageRx0("HDCP14 BCH Error Detected", ucSourceType);
            return _FALSE;
        }
    }
#endif
...
#if(_HDMI_HDCP14_BCH_ERROR_CHECK == _ON)
//--------------------------------------------------
// Description  : Stable Detect Check BCH
// Input Value  : enumBchChkState
// Output Value : False: bch error happen
//                True: bch error not happen
//--------------------------------------------------
bit ScalerTMDSStableDetectHdcp14BchErrorCheck(EnumHdmiHdcp14BchChkState enumBchChkState)
{
    switch(enumBchChkState)
    {
        case _HDMI_HDCP14_BCH_1ST_ACT_ENC_CHK_FAIL:

            // BCH Error already happened in FirstActiveProc
            // Lock Ri' to let next Ri check fail -> Force Source to Re-Auth
            ScalerSetDataPortBit(P2_C3_HDCP_AP, _P2_C4_PT_C1_PORT_PAGE2_HDCP_SIR, ~_BIT4, _BIT4);

            // Clear TimerEvenet
            ScalerTimerCancelTimerEvent(_SCALER_TIMER_EVENT_RX0_TMDS_HDCP14_BCH_ERR_CHK);

            SET_TMDS_HDCP14_BCH_CHK_STATE(_HDMI_HDCP14_BCH_NONE);

            return _FALSE;

        case _HDMI_HDCP14_BCH_AUTH_START:

            SET_TMDS_HDCP14_BCH_CHK_STATE(_HDMI_HDCP14_BCH_CHK_WINDOW_START);

            ScalerTimerActiveTimerEvent(1000, _SCALER_TIMER_EVENT_RX0_TMDS_HDCP14_BCH_ERR_CHK);

            return _TRUE;

        case _HDMI_HDCP14_BCH_CHK_WINDOW_END:
...
//--------------------------------------------------
// Description  : HDCP1.4 BCH Error Check Event
// Input Value  : None
// Output Value : None
//--------------------------------------------------
void ScalerTMDSHdcp14BchErrorCheckEvent(EnumScalerTimerEventID enumEventID)
{
    ...
    SET_TMDS_HDCP14_BCH_CHK_STATE(_HDMI_HDCP14_BCH_CHK_WINDOW_END);
}

//--------------------------------------------------
// Description  : Get FirstActiveProc BCH Error Check Result
// Input Value  : ucInputPort
// Output Value : False : BCH Error Happened
//              : True : BCH Error Not Happened
//--------------------------------------------------
bit ScalerTMDSGetFirstActiveHdcp14BchErrorCheck(BYTE ucInputPort)
{
    switch(ucInputPort)
...
#if(_D0_TMDS_SUPPORT == _ON)
        case _D0_INPUT_PORT:
            if(GET_TMDS_HDCP14_BCH_CHK_STATE() == _HDMI_HDCP14_BCH_1ST_ACT_ENC_CHK_FAIL)
            {
                return _FALSE;
            }
            else
...
//--------------------------------------------------
// Description  : Reset HDCP1.4 BCH Error Check
// Input Value  : ucInputPort
// Output Value : None
//--------------------------------------------------
void ScalerTMDSHdcp14BchErrorCheckReset(BYTE ucInputPort)
