...
#if(_HDMI21_VRR_SUPPORT == _ON)
BYTE ScalerTMDSRx0HdmiRsv0GetAVIInfo(EnumAviInfoContent enumContent);
#endif
...
WORD ScalerTMDSGetOvtVfreqParameter(BYTE ucFrIndex);
#endif
...
void ScalerTMDSDviVideoSetting(BYTE ucInputPort)
{
    ...
    ScalerSetDataPortBit(P2_C9_HDMI_AP, _P2_CA_PT_51_PORT_PAGE2_HDMI_ACRCR, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), 0x00);

    // Disable Down Sampling
    ScalerSetDataPortBit(P2_C9_HDMI_AP, _P2_CA_PT_50_PORT_PAGE2_HDMI_VCR, ~(_BIT6 | _BIT5 | _BIT3 | _BIT2 | _BIT1 | _BIT0), 0x00);
...
void ScalerTMDSFreqDetectDisable(void)
{
    ...
    ScalerSetBit(P2_EF_HDMI_ERROR_TH, ~(_BIT7 | _BIT6), 0x00);
    ...
    ScalerTMDSHdcp14BchErrorCheckReset(_D0_INPUT_PORT);
#endif
#if(_D1_TMDS_SUPPORT == _ON)
    ScalerTMDSHdcp14BchErrorCheckReset(_D1_INPUT_PORT);
#endif
#endif
}
//--------------------------------------------------
...
BYTE ScalerTMDSHdmiGetColorDepth(void)
{
    ...
    {
        if(ScalerGetBit(P2_B4_TMDS_DPC0, (_BIT3 | _BIT2 | _BIT1 | _BIT0)) == (_BIT2 | _BIT0))
        {
            return _COLOR_DEPTH_10_BITS;
        }
        else if(ScalerGetBit(P2_B4_TMDS_DPC0, (_BIT3 | _BIT2 | _BIT1 | _BIT0)) == (_BIT2 | _BIT1))
        {
            return _COLOR_DEPTH_12_BITS;
        }
        else if(ScalerGetBit(P2_B4_TMDS_DPC0, (_BIT3 | _BIT2 | _BIT1 | _BIT0)) == (_BIT2 | _BIT1 | _BIT0))
        {
...
void ScalerTMDSHdmiPacketDetect(bit bWait)
{
    ...
    else
    {
        ...
        if(ScalerGetBit(P2_CC_HDMI_GPVS, _BIT5) == _BIT5)
        {
            ...
            ScalerSetDataPortBit(P2_C9_HDMI_AP, _P2_CA_PT_51_PORT_PAGE2_HDMI_ACRCR, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT6 | _BIT2));
    ...
    if(bEnable == _ENABLE)
    {
        if(((ucWDType & _HDMI_AVI_INFO_VARIATION_WD) == _HDMI_AVI_INFO_VARIATION_WD) && (GET_DIGITAL_COLOR_INFO_CHANGE_RESET() == _TRUE))
        {
            if(ScalerGetDataPortBit(P2_C9_HDMI_AP, _P2_CA_PT_41_PORT_PAGE2_HDMI_VWDSR, _BIT0) == 0x00)
            {
                ...
                ScalerSetByte(P2_CC_HDMI_GPVS, 0x01);
...
#if(_HDMI21_VRR_SUPPORT == _ON)
//--------------------------------------------------
// Description  : Get HDMI AVI Info Contents from RSV0
// Input Value  : Content of Interest
// Output Value : Content Value
//--------------------------------------------------
BYTE ScalerTMDSRx0HdmiRsv0GetAVIInfo(EnumAviInfoContent enumContent)
    ...
    if(GET_HDMI_RX0_AVI_INFO_FRAME_RECEIVED() == _TRUE)
    {
        switch(enumContent)
        {
            case _AVI_INFO_COLOR_SPACE:
                ...
                return (ScalerGetDataPortBit(P2_CD_HDMI_PSAP, 0x52, (_BIT6 | _BIT5)) >> 5);

            case _AVI_INFO_COLORIMETRY:

                return (ScalerGetDataPortBit(P2_CD_HDMI_PSAP, 0x53, (_BIT7 | _BIT6)) >> 6);

            case _AVI_INFO_EXTENDED_COLORIMETRY:

                return (ScalerGetDataPortBit(P2_CD_HDMI_PSAP, 0x54, (_BIT6 | _BIT5 | _BIT4)) >> 4);

            case _AVI_INFO_ADD_EXTENDED_COLORIMETRY:

                return (ScalerGetDataPortBit(P2_CD_HDMI_PSAP, 0x5F, (_BIT7 | _BIT6 | _BIT5 | _BIT4)) >> 4);

            case _AVI_INFO_RGB_QUANTIZATION:

                return (ScalerGetDataPortBit(P2_CD_HDMI_PSAP, 0x54, (_BIT3 | _BIT2)) >> 2);

            case _AVI_INFO_YCC_QUANTIZATION:

                return (ScalerGetDataPortBit(P2_CD_HDMI_PSAP, 0x56, (_BIT7 | _BIT6)) >> 6);

            case _AVI_INFO_SCAN_INFO:

                return ScalerGetDataPortBit(P2_CD_HDMI_PSAP, 0x52, (_BIT1 | _BIT0));

            case _AVI_INFO_IT_CONTENT:

                return (ScalerGetDataPortBit(P2_CD_HDMI_PSAP, 0x54, _BIT7) >> 7);

            case _AVI_INFO_CONTENT_TYPE:

                return (ScalerGetDataPortBit(P2_CD_HDMI_PSAP, 0x56, (_BIT5 | _BIT4)) >> 4);

            case _AVI_INFO_PIXEL_REPETITION:

                return (ScalerGetDataPortBit(P2_CD_HDMI_PSAP, 0x56, (_BIT3 | _BIT2 | _BIT1 | _BIT0)));

            case _AVI_INFO_AR:

                return (ScalerGetDataPortBit(P2_CD_HDMI_PSAP, 0x53, (_BIT5 | _BIT4)) >> 4);

            case _AVI_INFO_VIC:

                return (ScalerGetDataPortBit(P2_CD_HDMI_PSAP, 0x55, (_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0)));

            case _AVI_INFO_FR:

                return ((ScalerGetDataPortBit(P2_CD_HDMI_PSAP, 0x60, _BIT6) >> 2) | (ScalerGetDataPortBit(P2_CD_HDMI_PSAP, 0x5F, (_BIT3 | _BIT2 | _BIT1 | _BIT0))));

            case _AVI_INFO_RID:

                return ScalerGetDataPortBit(P2_CD_HDMI_PSAP, 0x60, (_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0));
...
BYTE ScalerTMDSGetVicVfreqParameter(BYTE ucVicIndex)
{
    return tVIC_VFREQ_TABLE[ucVicIndex];
}
