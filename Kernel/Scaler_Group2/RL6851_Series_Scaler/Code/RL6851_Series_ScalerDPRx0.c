...
void ScalerDpRx0CrcCalculate(void)
{
    if(GET_DP_RX0_CRC_CALCULATE() == _TRUE)
    {
        ...
        {
            ScalerSetBit(PB5_70_DP_CRC_CTRL, ~(_BIT5 | _BIT4), _BIT4);
        }
        else
        {
            ScalerSetBit(PB5_70_DP_CRC_CTRL, ~(_BIT5 | _BIT4), 0x00);
        }

        // Start CRC Calculation
        ScalerSetBit(PB5_70_DP_CRC_CTRL, ~(_BIT7 | _BIT5), _BIT7);
...
void ScalerDpRx0SinkStatusSet(EnumDpSinkReceivePort enumReceiverPort, EnumDpSinkStatus enumInSync)
{
    BYTE ucSinkStatusBackup = ScalerDpRx0GetDpcdInfo(0x00, 0x02, 0x05);
    ...
    if(enumInSync == _DP_SINK_IN_SYNC)
    {
        if(enumReceiverPort == _DP_SINK_REVEICE_PORT0)
        {
            if((ucSinkStatusBackup & _BIT0) != _BIT0)
            {
                ...
                ScalerDpRx0SetDpcdValue(0x00, 0x02, 0x05, (ucSinkStatusBackup | _BIT0));
            }
        }
        else if(enumReceiverPort == _DP_SINK_REVEICE_PORT1)
        {
            if((ucSinkStatusBackup & _BIT1) != _BIT1)
            {
                ...
                ScalerDpRx0SetDpcdValue(0x00, 0x02, 0x05, (ucSinkStatusBackup | _BIT1));
            }
        }
    }
    else
...
void ScalerDpRx0ChangeHdcpDpcdCapability(bit bEn)
{
    ...
    else
    {
        ...
        ScalerSetBit(PB7_DA_AUX_FIFO_RST, ~(_BIT4 | _BIT2 | _BIT1 | _BIT0), 0x00);

        // Disable Write AKSV int
        ScalerSetBit(PB7_A5_AUX_HDCP_IRQ2, ~(_BIT6 | _BIT3 | _BIT2), 0x00);
