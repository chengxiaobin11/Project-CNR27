...
#define _NOTM_BOUND_SAVE_LENGTH     9
#endif
...
//--------------------------------------------------
#define _DISP_CALIB_FW_VERSION      0x03
...
//****************************************************************************************//
        ...
        else if(ucCalibrationStep == _DDCCI_DISP_CALIB_GET_PANEL_INFO)
        {
...
BYTE UserCommonDdcciDispCalibCalCheckSum(BYTE ucCheckType)
{
    ...
    switch(ucCheckType)
    {
        ...
        case _OCC_HDR_TYPE:
            ...
            for(usI = 0; usI < 411 * 3; usI++)
            {
                UserCommonFlashRead(_OGC_FLASH_BANK, ((((WORD)_OGC_FLASH_PAGE) << 12) + _OCC_HDR10_GAMMA10_ADDRESS + usI), 1, pData + 2);
                pData[0] += pData[2];
            }
...
void UserCommonDdcciDispCalibReplyResult(BYTE ucType)
{
    ...
    switch(ucType)
    {
        case _DISP_CALIB_START:
            ...
            g_pucDdcciTxBuf[_DDCCI_SOURCE] = _DDCCI_DEST_ADDRESS;
            g_pucDdcciTxBuf[_DDCCI_LENGTH] = _DDCCI_CONTROL_STATUS_FLAG | 0x04;
            g_pucDdcciTxBuf[_DDCCI_DISP_CALIB_OGCOCCDICOM_SUPPORT] = ((_HDR10_SUPPORT << 4) | (_DCIP3_NO_COMPRESSION_SUPPORT << 3) | (_OGC_DICOM_SUPPORT << 2) | (_OCC_SUPPORT << 1) | (_OGC_SUPPORT));
