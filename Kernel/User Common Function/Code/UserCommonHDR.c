...
BYTE OsdBackupDCRStatus = _DEF_DCR;
BYTE OsdBackupECOStatus = _DEF_ECO;
...
EnumHDRTargetStatus UserCommonHDRGetTargetStatus(EnumHDRFlowStatus enumHDRFlowStatus, EnumHDR10ModeDef enumHDR10ModeStatus, EnumFreeSyncSupport enumFreesyncIIModeStatus)
{
    ...
    switch(enumHDRFlowStatus)
    {
        ...
        case _HDR_FLOW_OSD_MODE_CHANGE:
			...
			if(enumHDR10ModeStatus == _HDR10_MODE_AUTO)
			{
				SET_OSD_DCR_STATUS(OsdBackupDCRStatus);
				SET_OSD_COLOR_EFFECT(OsdBackupECOStatus);
			}
...
EnumHDR10Status UserCommonHDRGetHDR10Status(void)
{
    ...
    else
    {
        ...
        {
        	if(UserInterfaceHDRGetHDR10ModeStatus(SysSourceGetInputPort()) == _HDR10_MODE_AUTO)
        	{
	        	OsdBackupDCRStatus = GET_OSD_DCR_STATUS();
				OsdBackupECOStatus = GET_OSD_COLOR_EFFECT();
				DebugMessageHDR("OsdBackupDCRStatus",OsdBackupDCRStatus);
				DebugMessageHDR("OsdBackupECOStatus",OsdBackupECOStatus);
        	}
			OsdHdrAdj();
            return _HDR10_ON;
        }
...
#endif
        {
	        ...
	        #if(Project_ID == ID_CHANGJIA_GS24S3F_HK2556E01_1P2H_M236HVR010_FHD144HZ_20210908)||(Project_ID == ID_MC_HK2556E01_1P2H_AUO_M250HAN01_8_LVDS_FHD165HZ_20211122)
				SET_OSD_GAMMA(_DEF_GAMMA);
			#endif
            return _HDR10_OFF;
        }
    }
...
#if 0
BYTE UserCommonHDRGetHDR10Status_1(void)
{
	return UserCommonHDRGetHDR10Status();
}
#endif
//--------------------------------------------------
...
#endif
#if(_DEF_HDR_540)
	usHDRInputGammaFlashAddress = _OGC_HDR10_IG_540_NOR_ADDRESS;
	usHDRColorMatrixFlashAddress = _OCC_HDR10_BT2020_MATRIX_ADDRESS;
#endif	
    UserCommonAdjustPCM(_PCM_HDR_2084, stHDR10Table.pucIGTable, stHDR10Table.pucOGTable, stHDR10Table.pucSRGBMatrix, stHDR10Table.ucBank, _OCC_COLORMATRIX_TABLE_SIZE);
}
...
void UserCommonHDRAdjustHDR10Contrast(BYTE ucContrast)
{
	...
	pusData[0] = 0x810;//0x7F0;
    pusData[1] = 0x830;//0x7F0;
    pusData[2] = 0x860;//0x800;
#else
    ...
    pusData[0] = (WORD)(((DWORD)pusData[3] * (ucContrast + 200) / 200) > 255) ? 255 : (WORD)((DWORD)pusData[3] * (ucContrast + 200) / 200);
    pusData[1] = pusData[0];
    pusData[2] = pusData[0];
