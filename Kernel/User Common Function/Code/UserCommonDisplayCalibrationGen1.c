...
#define _DISP_CALIB_GET_GD_SET      11
...
//--------------------------------------------------
#define _DISP_CALIB_FW_VERSION      0x0018
...
//****************************************************************************************//
        ...
        if(ucCalibrationStep == _DDCCI_DISP_CALIB_GET_DEFINE)
        {
            ...
            // Restore Backlight
            UserInterfaceAdjustBacklight(_BACKLIGHT_MAX);
#endif
        ...
        if(ucCalibrationStep == _DDCCI_DISP_CALIB_GET_FW_INFO)
        {
            ...
            else if(g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_FW_INFO] == _DDCCI_DISP_CALIB_GET_GD_DEFINE)
            {
                UserCommonDdcciDispCalibReplyResult(_DISP_CALIB_GET_GD_SET);
            }
            ScalerMcuDdcciReleaseScl();
            return;
        }
...
//****************************************************************************************//
        ...
        else if(ucCalibrationStep == _DDCCI_DISP_CALIB_DIMMING_SET_GD_HDR)
        {
            ...
            // Set Output Gamma
            ScalerColorOutputGammaEnable(_FUNCTION_OFF);
            ScalerColorPCMOutputGammaAdjust(g_pucDISP_CALIBRATION_FLASH + _DIMMING_OCC_GAMMA10_ADDRESS, _OCC_GAMMA_FLASH_BANK);

            // Set Color Matrix
            ScalerColorPCMSRGBAdjust(_SRGB_3_BIT_SHIFT_LEFT, g_pucDISP_CALIBRATION_FLASH + _OCC_HDR10_SRGB_MATRIX_ADDRESS + (g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_SET_HDR_COLOR_MATRIX] * _OCC_COLORMATRIX_TABLE_SIZE), _OGC_FLASH_BANK, _OCC_COLORMATRIX_TABLE_SIZE);
        ...
        else if(ucCalibrationStep == _DDCCI_DISP_CALIB_DIMMING_WRITE_FLASH)
        {
            ...
            if(g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_DIMMING_FLASH_FIRST] != 0)
            {
                g_usColorGDSavePos = 0;
            }

            switch(g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_DIMMING_FLASH_INDEX])
            {
            ...
            ucFlag &= UserCommonFlashWrite(_OGC_FLASH_BANK, usFlashAddr, g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_DIMMING_FLASH_LEN], &g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_DIMMING_FLASH_DATA]);
            g_usColorGDSavePos += g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_DIMMING_FLASH_LEN];
        }
        else if(ucCalibrationStep == _DDCCI_DISP_CALIB_DIMMING_SWITCH)
        {
            SET_ON_LINE_GD_HDR_CALIBRATION_STATUS(g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_DIMMING_SWITCH_INDEX]);
            if(g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_DIMMING_SWITCH_INDEX] == _CALIBRATION_ON)
            {
                ...
                if(UserCommonGlobalDimmingGetEnableStatus() == _GD_ENABLE)
                {
                    if(GET_ON_LINE_GD_HDR_CALIBRATION_STATUS() == _ON)
                    {
                        ...
                        ScalerColorPCMInputGammaAdjust(g_pucDISP_CALIBRATION_FLASH + _OGC_HDR10_IG_540_NOR_ADDRESS, _OGC_FLASH_BANK);  // tusHDR10_IG_ADDRESS_INDEX[UserCommonHDRGetHDR10LvType() * 2 + UserInterfaceHDRGetHDR10DarkEnhanceStatus()]
                        ScalerColorPCMSRGBAdjust(_SRGB_1_BIT_SHIFT_LEFT, (g_pucDISP_CALIBRATION_FLASH + UserCommonHDRGetHDR10ColorMatrix()), _OGC_FLASH_BANK, _OCC_COLORMATRIX_TABLE_SIZE);
                        ScalerColorPCMOutputGammaAdjust(g_pucDISP_CALIBRATION_FLASH + _DIMMING_OCC_GAMMA10_ADDRESS, _OCC_GAMMA_FLASH_BANK);
        ...
        {
            usR = ((WORD)g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_GENERAL_DPG_R_CODE_H] << 8) | ((WORD)g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_GENERAL_DPG_R_CODE_L]);
            usG = ((WORD)g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_GENERAL_DPG_G_CODE_H] << 8) | ((WORD)g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_GENERAL_DPG_G_CODE_L]);
            usB = ((WORD)g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_GENERAL_DPG_B_CODE_H] << 8) | ((WORD)g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_GENERAL_DPG_B_CODE_L]);
            ScalerDDomainPatternGenAdjustColor10bit(usR, usG, usB);
            ScalerDDomainPatternGenEnable(_ENABLE);
            ScalerDDomainBackgroundEnable(_DISABLE);
        }
...
#if(_GLOBAL_DIMMING_ONLINE_CALIBRATION_SUPPORT == _ON)
        case _GD_PWMLUT_TYPE:
            pucTemp[0] = UserCommonDdcciDispCalibCalCheckSum(_GD_PWMLUT_TYPE);
...
void UserCommonDdcciDispCalibReplyResult(BYTE ucType)
{
    ...
    switch(ucType)
    {
        ...
        case _DISP_CALIB_GET_FW_ADVANCE:
            ...
            g_pucDdcciTxBuf[_DDCCI_DISP_CALIB_ADVANCE_3] = (_GLOBAL_DIMMING_DARK_MODE_SUPPORT << 1) | (_GLOBAL_DIMMING_ONLINE_CALIBRATION_SUPPORT);
        ...
        case _DISP_CALIB_GET_GD_SET:
            ...
            g_pucDdcciTxBuf[_DDCCI_DISP_CALIB_GD_1] = _GLOBAL_DIMMING_DARK_MODE_MAX_GAIN;
            g_pucDdcciTxBuf[_DDCCI_DISP_CALIB_GD_2] = (_GD_PANEL_LV_MAX & 0xFF00) >> 8;
            g_pucDdcciTxBuf[_DDCCI_DISP_CALIB_GD_3] = _GD_PANEL_LV_MAX & 0xFF;
            g_pucDdcciTxBuf[_DDCCI_DISP_CALIB_GD_4] = (_GD_PANEL_LV_MIN & 0xFF00) >> 8;
            g_pucDdcciTxBuf[_DDCCI_DISP_CALIB_GD_5] = _GD_PANEL_LV_MIN & 0xFF;
            g_pucDdcciTxBuf[_DDCCI_DISP_CALIB_GD_6] = 0x00;
            g_pucDdcciTxBuf[_DDCCI_DISP_CALIB_GD_6 + 1] = UserCommonDdcciCalCheckSum();
