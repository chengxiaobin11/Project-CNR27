...
BYTE ScalerDpAuxJudgeHandler_EXINT0(void) using 1
{
    ...
    if(ScalerGetBit_EXINT(PB7_DC_AUX_IRQ_STATUS, _BIT7) == _BIT7)
    {
        ...
        if(ScalerGetBit_EXINT(PB7_FB_DUMMY_1, (_BIT3 | _BIT2)) == (_BIT3 | _BIT2))
        {
            ...
            // Fw handle if aux request include 00340-00359
            if((ucDpRxAuxCmdAddrEnd >= 0x40) && (ucDpRxAuxCmdAddrStart <= 0x59))
            {
                // Write Cmd Reply Ack and Backup Data
                if(ScalerGetBit_EXINT(PB7_D2_AUX_RX_CMD, (_BIT7 | _BIT6 | _BIT5 | _BIT4)) == _BIT7)
                {
                    // Wait Reply ACK
                    ScalerDpRx0AuxWaitRcv_EXINT0();
                    ...
                    for(ucCount = ucDpRxAuxCmdAddrStart; ucCount <= ucDpRxAuxCmdAddrEnd; ucCount++)
                    {
                        if((ucCount >= 0x44) && (ucCount <= 0x59))
                        {
                            g_pucDPRx0DpcdEDpHdr[ucCount - 0x40] = ScalerGetByte_EXINT(PB7_D6_AUX_RX_DATA);
                        }
                        else
                        {
                            ScalerGetByte_EXINT(PB7_D6_AUX_RX_DATA);
                        }
                    }
                }
                // Read Cmd Reply Data
                else if(ScalerGetBit_EXINT(PB7_D2_AUX_RX_CMD, (_BIT7 | _BIT6 | _BIT5 | _BIT4)) == (_BIT7 | _BIT4))
                {
                    // Wait Reply Defer
                    ScalerDpRx0AuxWaitRcv_EXINT0();

                    // Wait Next Retry
                    while((ScalerGetBit_EXINT(PB7_C7_TP1_OCCR, (_BIT2 | _BIT1 | _BIT0)) != _BIT1) && (ucTimeout--))
                    {
                        DELAY_5US_EXINT();
                    }

                    ucDpRxAuxCmdAddrStart = ScalerGetByte_EXINT(PB7_D4_AUX_RX_ADDR_L);
                    ucDpRxAuxCmdAddrEnd = ucDpRxAuxCmdAddrStart + (ScalerGetByte_EXINT(PB7_D5_AUX_RXLEN) & 0x0F);

                    // Double check aux read request include 00340-00359
                       ...
                       ((ucDpRxAuxCmdAddrEnd >= 0x40) && (ucDpRxAuxCmdAddrStart <= 0x59)))
                    {
                        ...
                        for(ucCount = ucDpRxAuxCmdAddrStart; ucCount <= ucDpRxAuxCmdAddrEnd; ucCount++)
                        {
                            if((ucCount >= 0x40) && (ucCount <= 0x59))
                            {
                                ScalerSetByte_EXINT(PB7_D9_AUX_TX_DATA, g_pucDPRx0DpcdEDpHdr[ucCount - 0x40]);
                            }
                            else
                            {
                                ScalerSetByte_EXINT(PB7_D9_AUX_TX_DATA, 0x00);
    ...
    if(ScalerGetBit_EXINT(PBA_DC_AUX_IRQ_STATUS, _BIT7) == _BIT7)
    {
        ...
        if(ScalerGetBit_EXINT(PBA_FB_DUMMY_1, (_BIT3 | _BIT2)) == (_BIT3 | _BIT2))
        {
            ...
            {
                // Write Cmd Reply Ack and Backup Data
                if(ScalerGetBit_EXINT(PBA_D2_AUX_RX_CMD, (_BIT7 | _BIT6 | _BIT5 | _BIT4)) == _BIT7)
                {
                    // Wait Reply ACK
                    ScalerDpRx1AuxWaitRcv_EXINT0();
                    ...
                    for(ucCount = ucDpRxAuxCmdAddrStart; ucCount <= ucDpRxAuxCmdAddrEnd; ucCount++)
                    {
                        if((ucCount >= 0x44) && (ucCount <= 0x59))
                        {
                            g_pucDPRx1DpcdEDpHdr[ucCount - 0x40] = ScalerGetByte_EXINT(PBA_D6_AUX_RX_DATA);
                        }
                        else
                        {
                            ScalerGetByte_EXINT(PBA_D6_AUX_RX_DATA);
                        }
                    }
                }
                // Read Cmd Reply Data
                else if(ScalerGetBit_EXINT(PBA_D2_AUX_RX_CMD, (_BIT7 | _BIT6 | _BIT5 | _BIT4)) == (_BIT7 | _BIT4))
                {
                    // Wait Reply Defer
                    ScalerDpRx1AuxWaitRcv_EXINT0();

                    // Wait Next Retry
                    while((ScalerGetBit_EXINT(PBA_C7_TP1_OCCR, (_BIT2 | _BIT1 | _BIT0)) != _BIT1) && (ucTimeout--))
                    {
                        DELAY_5US_EXINT();
                    }

                    ucDpRxAuxCmdAddrStart = ScalerGetByte_EXINT(PBA_D4_AUX_RX_ADDR_L);
                    ucDpRxAuxCmdAddrEnd = ucDpRxAuxCmdAddrStart + (ScalerGetByte_EXINT(PBA_D5_AUX_RXLEN) & 0x0F);
                       ...
                       ((ucDpRxAuxCmdAddrEnd >= 0x40) && (ucDpRxAuxCmdAddrStart <= 0x59)))
                    {
                        ...
                        for(ucCount = ucDpRxAuxCmdAddrStart; ucCount <= ucDpRxAuxCmdAddrEnd; ucCount++)
                        {
                            if((ucCount >= 0x40) && (ucCount <= 0x59))
                            {
                                ScalerSetByte_EXINT(PBA_D9_AUX_TX_DATA, g_pucDPRx1DpcdEDpHdr[ucCount - 0x40]);
                            }
                            else
                            {
                                ScalerSetByte_EXINT(PBA_D9_AUX_TX_DATA, 0x00);
