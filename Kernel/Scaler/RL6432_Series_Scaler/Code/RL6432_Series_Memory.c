...
void ScalerMemoryFrameSyncSetDt(BYTE ucNum)
{
    ...
    if(GET_FREESYNC_ENABLED() == _TRUE)
    {
        ...
        usCheckDVstRising = (DWORD)g_stMDomainInputData.usHTotal * 9 / 10;

        if(g_stMDomainInputData.usVHeight <= g_stMDomainOutputData.usVHeight)
        {
            if(ulIVStoDVSPixels > usCheckDVstRising)
            {
                g_stMDomainOutputData.usVStart += 1;
        ...
        {
            // preread = 0, oden start request = IVS2DVS + 1 D-line
            WORD usAddPreread = ulIVStoDVSPixels + ((DWORD)g_stMDomainInputData.usVHeight * g_stMDomainInputData.usHTotal / g_stMDomainOutputData.usVHeight);

            if(usAddPreread > usCheckDVstRising)
            {
                if(bDVstAdd == _TRUE)
                {
                    g_stMDomainOutputData.usVStart += 1;
                ...
                {
                    // no more DVst can be add, modify IVS2DVS delay
                    ulIVStoDVSPixels = 0;
                }

                DebugMessageScaler("VSD DRR IVS2DVS delay close to 1 line, modify IVS2DVS delay", ulIVStoDVSPixels);
            }
        }
    }
    else
