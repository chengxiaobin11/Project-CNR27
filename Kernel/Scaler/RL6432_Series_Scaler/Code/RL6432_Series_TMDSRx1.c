...
volatile EnumHdmiHdcp14BchChkState g_enumTMDSRx1Hdcp14BchChkState;
#endif
...
#if(_HDMI_HDCP14_BCH_ERROR_CHECK == _ON)
bit ScalerTMDSRx1StableDetectHdcp14BchErrorCheck(EnumHdmiHdcp14BchChkState enumBchChkState);
void ScalerTMDSRx1Hdcp14BchErrorCheckEvent(void);
void ScalerTMDSRx1FirstActiveProcHdcp14BchErrorCheck(void);
bit ScalerTMDSRx1GetFirstActiveHdcp14BchErrorCheck(void);
void ScalerTMDSRx1Hdcp14BchErrorCheckReset(void);
#if(_TMDS_HDCP_2_2_SUPPORT == _ON)
#if((_D1_INPUT_PORT_TYPE == _D1_HDMI_PORT) || (_D1_INPUT_PORT_TYPE == _D1_MHL_PORT))
void ScalerTMDSRx1Hdcp14BchErrorCheckReset_EXINT0(void);
#endif
#endif
#endif
...
#if(_HDMI_HDCP14_BCH_ERROR_CHECK == _ON)
        // Unlock Ri'
        ScalerSetByte_EXINT(P71_C3_HDCP_AP, _P71_C4_PT_C1_PORT_PAGE71_HDCP_SIR);
        ScalerSetBit_EXINT(P71_C4_HDCP_DP, ~_BIT4, 0x00);

        // Clear TimerEvenet
        ScalerTimerCancelTimerEvent_EXINT0(_SCALER_TIMER_EVENT_RX1_TMDS_HDCP14_BCH_ERR_CHK);

        SET_TMDS_RX1_HDCP14_BCH_CHK_STATE(_HDMI_HDCP14_BCH_AUTH_START);
#endif
...
#if(_HDMI_HDCP14_BCH_ERROR_CHECK == _ON)
    // Reset HDCP1.4 BCH Error Check
    ScalerTMDSRx1Hdcp14BchErrorCheckReset();
#endif
}
...
BYTE ScalerTMDSRx1TMDSScanInputPort(BYTE ucInputType)
{
    ...
    if(GET_TMDS_RX1_PHY_SET() == _TRUE)
    {
        ...
        if(ucTMDSType == _SOURCE_HDMI)
        {
            ...
            {
                if(ScalerGetDataPortBit(P71_C9_HDMI_AP, _P71_CA_PT_02_PORT_PAGE71_HDMI_BCHCR, _BIT2) == _BIT2)
                {
                    // Clr BCH Error Flag
                    ScalerSetDataPortBit(P71_C9_HDMI_AP, _P71_CA_PT_02_PORT_PAGE71_HDMI_BCHCR, ~_BIT2, 0x00);
            ...
            if(ucBchErrorCount >= 6)
            {
                ...
                ScalerSetDataPortBit(P71_C3_HDCP_AP, _P71_C4_PT_C1_PORT_PAGE71_HDCP_SIR, ~_BIT4, _BIT4);

                DebugMessageRx1("BCH Error Detected", ucBchErrorCount);
...
bit ScalerTMDSRx1TMDSStableDetect(BYTE ucSourceType)
{
    ...
    if(ucSourceType == _SOURCE_HDMI)
    {
        ...
        if(ScalerTMDSRx1StableDetectHdcp14BchErrorCheck(GET_TMDS_RX1_HDCP14_BCH_CHK_STATE()) == _FALSE)
        {
            DebugMessageRx1("HDCP14 BCH Error Detected", ucSourceType);
            return _FALSE;
        }
    }
#endif
...
bit ScalerTMDSRx1StableDetectHdcp14BchErrorCheck(EnumHdmiHdcp14BchChkState enumBchChkState)
{
    switch(enumBchChkState)
    {
        case _HDMI_HDCP14_BCH_1ST_ACT_ENC_CHK_FAIL:
            ...
            // Clear TimerEvenet
            ScalerTimerCancelTimerEvent(_SCALER_TIMER_EVENT_RX1_TMDS_HDCP14_BCH_ERR_CHK);

            SET_TMDS_RX1_HDCP14_BCH_CHK_STATE(_HDMI_HDCP14_BCH_NONE);

            return _FALSE;

        case _HDMI_HDCP14_BCH_AUTH_START:

            ScalerTimerActiveTimerEvent(1000, _SCALER_TIMER_EVENT_RX1_TMDS_HDCP14_BCH_ERR_CHK);

            SET_TMDS_RX1_HDCP14_BCH_CHK_STATE(_HDMI_HDCP14_BCH_CHK_WINDOW_START);
        ...
        case _HDMI_HDCP14_BCH_CHK_WINDOW_END:
            ...
            if(ScalerGetDataPortBit(P71_C9_HDMI_AP, _P71_CA_PT_02_PORT_PAGE71_HDMI_BCHCR, _BIT2) == _BIT2)
            {
                ...
                // Disable RGB/clock Output to avoid garbage
                ScalerSetBit(P71_A6_TMDS_OUTCTL, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3), 0x00);
...
void ScalerTMDSRx1Hdcp14BchErrorCheckEvent(void)
{
    SET_TMDS_RX1_HDCP14_BCH_CHK_STATE(_HDMI_HDCP14_BCH_CHK_WINDOW_END);
}

//--------------------------------------------------
// Description  : Check BCH Error in FirstActiveProc
// Input Value  : None
// Output Value : None
//--------------------------------------------------
void ScalerTMDSRx1FirstActiveProcHdcp14BchErrorCheck(void)
{
    ...
    usRemainTime = 80;
    while(GET_TMDS_RX1_HDCP14_BCH_CHK_STATE() == _HDMI_HDCP14_BCH_NONE)
    {
        ScalerTimerDelayXms(ucCheckTime);
        usRemainTime -= ucCheckTime;
        if((GET_TMDS_RX1_HDCP14_BCH_CHK_STATE() == _HDMI_HDCP14_BCH_AUTH_START) || (usRemainTime < ucCheckTime))
        {
            break;
        }
    }

    // Check BCH Error for HDCP1.4 in _SCALER_TIMER_EVENT_RX1_TMDS_HDCP14_BCH_ERR_CHK window
    switch(GET_TMDS_RX1_HDCP14_BCH_CHK_STATE())
    {
        case _HDMI_HDCP14_BCH_AUTH_START:
        case _HDMI_HDCP14_BCH_CHK_WINDOW_START:

            if(GET_TMDS_RX1_HDCP14_BCH_CHK_STATE() == _HDMI_HDCP14_BCH_CHK_WINDOW_START)
            {
                usRemainTime = ScalerTimerGetEventRemainTime(_SCALER_TIMER_EVENT_RX1_TMDS_HDCP14_BCH_ERR_CHK);
            }
            else
            {
                usRemainTime = 1000;
            ...
            ScalerGetDataPortByte(P71_C3_HDCP_AP, _P71_C4_PT_C0_PORT_PAGE71_HDCP_FCR, 1, &pucHDCP14FrmCnt[0], _NON_AUTOINC);
            while(usRemainTime >= ucCheckTime)
            {
                ScalerTimerDelayXms(ucCheckTime);
                ScalerGetDataPortByte(P71_C3_HDCP_AP, _P71_C4_PT_C0_PORT_PAGE71_HDCP_FCR, 1, &pucHDCP14FrmCnt[1], _NON_AUTOINC);
            ...
            if(ScalerGetDataPortBit(P71_C9_HDMI_AP, _P71_CA_PT_02_PORT_PAGE71_HDMI_BCHCR, _BIT2) == _BIT2)
            {
                ...
                SET_TMDS_RX1_HDCP14_BCH_CHK_STATE(_HDMI_HDCP14_BCH_1ST_ACT_ENC_CHK_FAIL);
                DebugMessageRx1("FirstActive HDCP14 BCH Error Detected", 0x00);
            }
            else
            {
                SET_TMDS_RX1_HDCP14_BCH_CHK_STATE(_HDMI_HDCP14_BCH_1ST_ACT_ENC_CHK_PASS);
            }
...
bit ScalerTMDSRx1GetFirstActiveHdcp14BchErrorCheck(void)
{
    if(GET_TMDS_RX1_HDCP14_BCH_CHK_STATE() == _HDMI_HDCP14_BCH_1ST_ACT_ENC_CHK_FAIL)
    {
        return _FALSE;
    }
    else
...
#if((_D1_INPUT_PORT_TYPE == _D1_HDMI_PORT) || (_D1_INPUT_PORT_TYPE == _D1_MHL_PORT))
//--------------------------------------------------
// Description  : Reset HDCP1.4 BCH Error Check (EXINT0 Only)
// Input Value  : None
// Output Value : None
//--------------------------------------------------
void ScalerTMDSRx1Hdcp14BchErrorCheckReset_EXINT0(void) using 1
{
    // Clear TimerEvenet
    ScalerTimerCancelTimerEvent_EXINT0(_SCALER_TIMER_EVENT_RX1_TMDS_HDCP14_BCH_ERR_CHK);
