...
volatile EnumHdmiHdcp14BchChkState g_enumTMDSRx2Hdcp14BchChkState;
#endif
...
#if(_HDMI_HDCP14_BCH_ERROR_CHECK == _ON)
bit ScalerTMDSRx2StableDetectHdcp14BchErrorCheck(EnumHdmiHdcp14BchChkState enumBchChkState);
void ScalerTMDSRx2Hdcp14BchErrorCheckEvent(void);
void ScalerTMDSRx2FirstActiveProcHdcp14BchErrorCheck(void);
bit ScalerTMDSRx2GetFirstActiveHdcp14BchErrorCheck(void);
void ScalerTMDSRx2Hdcp14BchErrorCheckReset(void);
#if(_TMDS_HDCP_2_2_SUPPORT == _ON)
#if((_D2_INPUT_PORT_TYPE == _D2_HDMI_PORT) || (_D2_INPUT_PORT_TYPE == _D2_MHL_PORT))
void ScalerTMDSRx2Hdcp14BchErrorCheckReset_EXINT0(void);
#endif
#endif
#endif
...
void ScalerTMDSRx2IntHandler_EXINT0(void) using 1
{
    ...
    if((ScalerGetBit_EXINT(P71_A9_TMDS_ABC0, _BIT7) == _BIT7) && (ScalerGetBit_EXINT(P71_AA_TMDS_ABC1, _BIT4) == _BIT4))
    {
        ...
        // Clear TimerEvenet
        ScalerTimerCancelTimerEvent_EXINT0(_SCALER_TIMER_EVENT_RX2_TMDS_HDCP14_BCH_ERR_CHK);

        SET_TMDS_RX2_HDCP14_BCH_CHK_STATE(_HDMI_HDCP14_BCH_AUTH_START);
#endif
...
void ScalerTMDSRx2FreqDetectDisable(void)
{
    ...
    ScalerTMDSRx2Hdcp14BchErrorCheckReset();
#endif
}
    ...
    if(GET_TMDS_RX2_PHY_SET() == _TRUE)
    {
        ...
        if(ucTMDSType == _SOURCE_HDMI)
        {
            ...
            if(ucBchErrorCount >= 6)
            {
                ...
                DebugMessageRx2("BCH Error Detected", ucBchErrorCount);
...
bit ScalerTMDSRx2TMDSStableDetect(BYTE ucSourceType)
{
    ...
    if(ucSourceType == _SOURCE_HDMI)
    {
        ...
        if(ScalerTMDSRx2StableDetectHdcp14BchErrorCheck(GET_TMDS_RX2_HDCP14_BCH_CHK_STATE()) == _FALSE)
        {
            DebugMessageRx2("HDCP14 BCH Error Detected", ucSourceType);
            return _FALSE;
        }
    }
#endif
...
bit ScalerTMDSRx2StableDetectHdcp14BchErrorCheck(EnumHdmiHdcp14BchChkState enumBchChkState)
{
    switch(enumBchChkState)
    {
        case _HDMI_HDCP14_BCH_1ST_ACT_ENC_CHK_FAIL:
            ...
            // Clear TimerEvenet
            ScalerTimerCancelTimerEvent(_SCALER_TIMER_EVENT_RX2_TMDS_HDCP14_BCH_ERR_CHK);

            SET_TMDS_RX2_HDCP14_BCH_CHK_STATE(_HDMI_HDCP14_BCH_NONE);

            return _FALSE;

        case _HDMI_HDCP14_BCH_AUTH_START:

            ScalerTimerActiveTimerEvent(1000, _SCALER_TIMER_EVENT_RX2_TMDS_HDCP14_BCH_ERR_CHK);

            SET_TMDS_RX2_HDCP14_BCH_CHK_STATE(_HDMI_HDCP14_BCH_CHK_WINDOW_START);
...
void ScalerTMDSRx2Hdcp14BchErrorCheckEvent(void)
{
    SET_TMDS_RX2_HDCP14_BCH_CHK_STATE(_HDMI_HDCP14_BCH_CHK_WINDOW_END);
}
...
void ScalerTMDSRx2FirstActiveProcHdcp14BchErrorCheck(void)
{
    ...
    usRemainTime = 80;
    while(GET_TMDS_RX2_HDCP14_BCH_CHK_STATE() == _HDMI_HDCP14_BCH_NONE)
    {
        ScalerTimerDelayXms(ucCheckTime);
        usRemainTime -= ucCheckTime;
        if((GET_TMDS_RX2_HDCP14_BCH_CHK_STATE() == _HDMI_HDCP14_BCH_AUTH_START) || (usRemainTime < ucCheckTime))
        {
            break;
        }
    }

    // Check BCH Error for HDCP1.4 in _SCALER_TIMER_EVENT_RX2_TMDS_HDCP14_BCH_ERR_CHK window
    switch(GET_TMDS_RX2_HDCP14_BCH_CHK_STATE())
    {
        case _HDMI_HDCP14_BCH_AUTH_START:
        case _HDMI_HDCP14_BCH_CHK_WINDOW_START:

            if(GET_TMDS_RX2_HDCP14_BCH_CHK_STATE() == _HDMI_HDCP14_BCH_CHK_WINDOW_START)
            {
                usRemainTime = ScalerTimerGetEventRemainTime(_SCALER_TIMER_EVENT_RX2_TMDS_HDCP14_BCH_ERR_CHK);
            }
            else
            {
                usRemainTime = 1000;
            ...
            if(ScalerGetDataPortBit(P71_C9_HDMI_AP, _P71_CA_PT_02_PORT_PAGE71_HDMI_BCHCR, _BIT2) == _BIT2)
            {
                ...
                SET_TMDS_RX2_HDCP14_BCH_CHK_STATE(_HDMI_HDCP14_BCH_1ST_ACT_ENC_CHK_FAIL);
                DebugMessageRx2("FirstActive HDCP14 BCH Error Detected", 0x00);
            }
            else
            {
                SET_TMDS_RX2_HDCP14_BCH_CHK_STATE(_HDMI_HDCP14_BCH_1ST_ACT_ENC_CHK_PASS);
            }
...
bit ScalerTMDSRx2GetFirstActiveHdcp14BchErrorCheck(void)
{
    if(GET_TMDS_RX2_HDCP14_BCH_CHK_STATE() == _HDMI_HDCP14_BCH_1ST_ACT_ENC_CHK_FAIL)
    {
        return _FALSE;
    }
    else
...
// Output Value : None
//--------------------------------------------------
void ScalerTMDSRx2Hdcp14BchErrorCheckReset_EXINT0(void) using 1
{
    // Clear TimerEvenet
    ScalerTimerCancelTimerEvent_EXINT0(_SCALER_TIMER_EVENT_RX2_TMDS_HDCP14_BCH_ERR_CHK);
