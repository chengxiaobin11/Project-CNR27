...
#endif
void ScalerDpRx1Initial(EnumDpVersionType enumDpVersion, EnumDpLaneCount enumDpLaneCount, EnumDPMSTPort enumDpRxMstPort, EnumFreeSyncSupport enumFreeSyncSupport, EnumAudioDpDpcdSupport enumAudioSupport, EnumDpHdrSupport enumDpHdrSupport);
...
void ScalerDpRx1HdrDpcdSetting(void);
#endif
#endif
...
void ScalerDpRx1Initial(EnumDpVersionType enumDpVersion, EnumDpLaneCount enumDpLaneCount, EnumDPMSTPort enumDpRxMstPort, EnumFreeSyncSupport enumFreeSyncSupport, EnumAudioDpDpcdSupport enumAudioSupport, EnumDpHdrSupport enumDpHdrSupport)
{
    ...
    ScalerDpRx1DpcdInitial(enumDpVersion, enumDpLaneCount, enumDpRxMstPort, enumFreeSyncSupport, enumDpHdrSupport);
...
void ScalerDpRx1SinkStatusSet(EnumDpSinkReceivePort enumReceiverPort, EnumDpSinkStatus enumInSync)
{
    BYTE ucSinkStatusBackup = ScalerDpRx1GetDpcdInfo(0x00, 0x02, 0x05);

    if(enumInSync == _DP_SINK_IN_SYNC)
    {
        if(enumReceiverPort == _DP_SINK_REVEICE_PORT0)
        {
            if((ucSinkStatusBackup & _BIT0) != _BIT0)
            {
                // Set Receive Port 0 in Sync
                ScalerDpRx1SetDpcdValue(0x00, 0x02, 0x05, (ucSinkStatusBackup | _BIT0));
            }
        }
        else if(enumReceiverPort == _DP_SINK_REVEICE_PORT1)
        {
            if((ucSinkStatusBackup & _BIT1) != _BIT1)
            {
                // Set Receive Port 1 in Sync
                ScalerDpRx1SetDpcdValue(0x00, 0x02, 0x05, (ucSinkStatusBackup | _BIT1));
            }
        }
    }
    else
...
void ScalerDpRx1HDCPHandler(void)
{
...
#endif
    {
        ...
        {
            // In MST mode
            if(ScalerDpGetSrcMstEnable(_D1_INPUT_PORT) == _ENABLE)
            {
    ...
    switch(enumBStatusType)
    {
        case _DP_HDCP_BSTATUS_LINK_INTEGRITY_FAIL:
            ...
            if(GET_DP_RX1_HDCP_MODE() == _HDCP_14)
            {
                ...
                if(ScalerDpRxMstGetSrcMsgEnable(_D1_INPUT_PORT) == _ENABLE)
                {
...
bit ScalerDpRx1DisplayFormatSetting(void)
{
    ...
    DebugMessageRx1("OOR Detect for Low Frame Rate: IVfreq", usIVfreq_OOR);

    if(usIVfreq_OOR <= 200)
    {
        SET_DP_RX1_JUDGE_OOR();

        DebugMessageRx1("Judge for OOR Case !!!!!!!!", 0);
    }
    else
    {
        CLR_DP_RX1_JUDGE_OOR();
    }
#endif
...
void ScalerDpRx1HdrDpcdSetting(void)
{
    EnumDpHdrSupport enumDpHdrSupport = ScalerDpRxHdrGetSupport(_D1_INPUT_PORT);

    if((enumDpHdrSupport == _DP_HDR_SUPPORT_ON) && (GET_DP_RX_D1_PORT_TARGET_VERSION() >= _DP_VERSION_1_4))
    {
        // Clear WildCard Flag
        ScalerSetBit(PBA_FB_DUMMY_1, ~(_BIT7 | _BIT5 | _BIT3 | _BIT2 | _BIT1), _BIT3);
        ...
        memset(g_pucDPRx1DpcdEDpHdr, 0, sizeof(g_pucDPRx1DpcdEDpHdr));

        // Initial Intel EDP HDR CAPS
        g_pucDPRx1DpcdEDpHdr[0] = 0x01;
        g_pucDPRx1DpcdEDpHdr[1] = 0x53;
        g_pucDPRx1DpcdEDpHdr[2] = 0x00;
        g_pucDPRx1DpcdEDpHdr[3] = 0x00;
#endif
    }
    else
    {
