...
void ScalerIDomainVgipSetInputCapture(void)
{
    ...
    else
    {
        ...
        // Check IVst + IVheight < IVtotal, try to modify VGIP V capture start & height
        if((g_stMDomainInputData.usVStart + g_stMDomainInputData.usVHeight) > g_stMDomainInputData.usVTotal)
        {
            if(g_stMDomainInputData.usVTotal > (g_stMDomainInputData.usVHeight + _VGIP_V_FRONT_PORCH_MIN_VALUE))
            {
                g_stMDomainInputData.usVStart = g_stMDomainInputData.usVTotal - (g_stMDomainInputData.usVHeight + _VGIP_V_FRONT_PORCH_MIN_VALUE);

                if(g_stMDomainInputData.usVStart < _VGIP_IV_CAPTURE_MIN_VALUE)
                {
                    g_stMDomainInputData.usVHeight -= (_VGIP_IV_CAPTURE_MIN_VALUE - g_stMDomainInputData.usVStart);
                    g_stMDomainInputData.usVHeight -= (g_stMDomainInputData.usVHeight % 2);
                    g_stMDomainInputData.usVStart = _VGIP_IV_CAPTURE_MIN_VALUE;

                    // Set VGIP V Capture Height
                    ScalerVgipSetCaptureVHeight(g_stMDomainInputData.usVHeight);
                    DebugMessageScaler("[Warning] VGIP V start should >= _VGIP_IV_CAPTURE_MIN_VALUE, try to modify VGIP V capture height.", g_stMDomainInputData.usVHeight);
                }
            }
            else
            {
                g_stMDomainInputData.usVHeight = g_stMDomainInputData.usVTotal - _VGIP_IV_CAPTURE_MIN_VALUE - _VGIP_V_FRONT_PORCH_MIN_VALUE;
                g_stMDomainInputData.usVHeight -= (g_stMDomainInputData.usVHeight % 2);
                g_stMDomainInputData.usVStart = _VGIP_IV_CAPTURE_MIN_VALUE;
        ...
        else if(g_stMDomainInputData.usVStart + g_stMDomainInputData.usVHeight + _VGIP_V_FRONT_PORCH_MIN_VALUE >= g_stMDomainInputData.usVTotal)
