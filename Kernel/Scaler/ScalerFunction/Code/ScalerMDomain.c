...
#if(_PANEL_TYPE == _AUO_M250HTN01_0_VBY_240HZ)||(_PANEL_TYPE == _AUO_M270HTN02_0_VBY_240HZ)
	if(GET_INPUT_TIMING_VFREQ()<1450)
		return _FALSE;
#elif(_PANEL_TYPE == _CSOT_SG3151B05_6_VBY1_240HZ)
	if(GET_INPUT_TIMING_VFREQ()<760)
		return _FALSE;
#elif(_PANEL_TYPE == _AUO_M270DAN06_6_QHD165HZ)	
	if(GET_INPUT_TIMING_VFREQ()<690)
		return _FALSE;
#elif(_PANEL_TYPE == _AUO_M315DVR01_0_EDP_165HZ)||(_PANEL_TYPE == _AUO_M315DVR01_9_EDP_165HZ)
			if(GET_INPUT_TIMING_VFREQ()<690)
				return _FALSE;
#elif(_PANEL_TYPE == _AUO_M270KCJ_K7B_VBY_QHD)	
			if(GET_INPUT_TIMING_VFREQ()<660)
		     	return _FALSE;
#elif(_PANEL_TYPE == _LG_LM270WR2_EDP_UHD_60HZ)	
			if((GET_INPUT_TIMING_HWIDTH() <2000)||(GET_INPUT_TIMING_VFREQ()<480))
				return _FALSE;
#elif 0//(_PANEL_TYPE == _AUO_M250HAN01_6_EDP_FHD165HZ)	
		if(GET_INPUT_TIMING_VFREQ()<740)
			return _FALSE;
...
void ScalerMDomainODConfig(void)
{
    ...
    SET_OD_USE_BIT(_OFF);
	DebugMessageFRC("GET_MEMORY_CONFIG", GET_MEMORY_CONFIG());
...
WORD ScalerMDomainGetODMemoryBW(void)
{
    ...
    // OD BW
    PDATA_DWORD(2) = GET_DWORD_MUL_DIV(((DWORD)g_stMDomainOutputData.usVTotal * g_stMDomainOutputData.usHWidth), ((DWORD)GET_OD_USE_BIT() * g_usDVFreq), ((DWORD)_MEMORY_BUS_WIDTH * 10000)) +
                     GET_DWORD_MUL_DIV(((DWORD)g_stMDomainOutputData.usVTotal * g_stMDomainOutputData.usHWidth), ((DWORD)GET_OD_USE_BIT() * g_usDVFreq), ((DWORD)_MEMORY_BUS_WIDTH * 10000)) * ((_DCLK_SPREAD_RANGE / _DPLL_SSCG_DIV_MODE) + _DCLK_SPREAD_RANGE_MARGIN) / 4 / 100;
