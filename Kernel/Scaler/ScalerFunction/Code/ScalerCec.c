...
void ScalerCecRxReadOutFromBackupFifo(void)
{
    ...
    if((GET_CEC_RX_READ_DONE() == _FALSE) && (GET_CEC_RX_MSG_ANALYZING() == _FALSE) && (g_ucCecRxBackupFifoWaterLevel != 0x00))
    {
        ...
        if(g_ucCecRxBackupFifoWaterLevel < _CEC_RX_BACKUP_FIFO_SIZE - 10)
        {
            ...
            ScalerSetBit(P18_A6_CEC1_CR0_7, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT2), 0x00);
        }
...
void ScalerCecRxWriteIntoBackupFifo(void)
{
    ...
    if((_CEC_RX_BACKUP_FIFO_SIZE - _CEC_RX_BACKUP_FIFO_OVERFLOW_BUFFER - g_ucCecRxBackupFifoWaterLevel) < (ScalerGetBit(P18_B1_CEC1_RXCR0_2, (_BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0)) + 2))
    {
...
void ScalerCecRxClrIMsgAnalyzingFlag(void)
{
    ...
    // User Read out (Pop) one CEC message, so processing (Index - 1)
...
void ScalerCecRxWriteIntoBackupFifo_EXINT0(void) using 1
{
    ...
    // (BackFifoSize - overflow Buffer) - Waterlevel < Rx Read count. means Not enough space for BackupFIFO
    // Both situation will set CEC Rx force Nack for Next CEC frame.
    if((_CEC_RX_BACKUP_FIFO_SIZE - _CEC_RX_BACKUP_FIFO_OVERFLOW_BUFFER - g_ucCecRxBackupFifoWaterLevel) < (ScalerGetBit_EXINT(P18_B1_CEC1_RXCR0_2, (_BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0)) + 2))
    {
        // Set CEC Rx force Nack/ Negative Ack
