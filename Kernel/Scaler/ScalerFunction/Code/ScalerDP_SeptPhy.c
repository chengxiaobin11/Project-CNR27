...
#endif
void ScalerDpInitial(BYTE ucInputPort, EnumDpVersionType enumDpVersion, EnumDpLaneCount enumDpLaneCount, EnumDPMSTPort enumDpRxMstPort, EnumFreeSyncSupport enumFreeSyncSupport, EnumAudioDpDpcdSupport enumAudioSupport, EnumDpHdrSupport enumDpHdrSupport);
...
#if((_DP_AUX_RX_DPCD_EXT_CAP_WILDCARD_SUPPORT == _ON) || (_DP_AUX_RX_DPCD_HDR_CAP_WILDCARD_SUPPORT == _ON))
...
void ScalerDpRxHdrSupportSwitch(BYTE ucInputPort, EnumDpHdrSupport enumDpHdrSupport);
EnumDpHdrSupport ScalerDpRxHdrGetSupport(BYTE ucInputPort);
void ScalerDpRxHdrDpcdSetting(BYTE ucInputPort);
#endif
#endif

bit ScalerDpRxChPowerDownResetCheck(BYTE ucInputPort);
bit ScalerDpRxGetLinkTrainingPass(BYTE ucInputPort);
BYTE ScalerDpRxGetLTStatus(BYTE ucInputPort);
#endif
...
bit ScalerDpGetSrcMstEnable(BYTE ucInputPort)
{
    if(ScalerDpRxGetDpcdBitInfo(ucInputPort, 0x00, 0x00, 0x21, _BIT0) == _BIT0)
    {
        if(ScalerDpRxGetDpcdBitInfo(ucInputPort, 0x00, 0x01, 0x11, _BIT0) == _BIT0)
        {
            return _ENABLE;
        }
    }
        ...
        case _POWER_ACTION_NORMAL_TO_FAKE_OFF:
            ...
            if(ScalerDpCheckDCOffHpdToggle(enumPowerAction) == _TRUE)
            {
...
#else
                    if((GET_DP_DC_OFF_HPD_HIGH() == _FALSE) ||
                       ((enumPowerAction == _POWER_ACTION_NORMAL_TO_OFF) &&
                        (GET_DP_RX0_HDCP_CHECK_ENABLE() == _TRUE)))
#endif
                    {
                        ScalerDpRx0SetHotPlugEvent(_DP_HPD_LOW_RESET_AUX);
                    }
...
#endif
                {
                    ...
                    if((GET_DP_DC_OFF_HPD_HIGH() == _FALSE) ||
                       ((enumPowerAction == _POWER_ACTION_NORMAL_TO_OFF) &&
                        (GET_DP_RX1_HDCP_CHECK_ENABLE() == _TRUE)))
#endif
                    {
                        ScalerDpRx1SetHotPlugEvent(_DP_HPD_LOW_RESET_AUX);
                    }
...
#if(_D0_DP_EXIST == _ON)
        case _D0_INPUT_PORT:
            ScalerDpRx0Initial(enumDpVersion, enumDpLaneCount, enumDpRxMstPort, enumFreeSyncSupport, enumAudioSupport, enumDpHdrSupport);
            break;
#endif

#if(_D1_DP_EXIST == _ON)
        case _D1_INPUT_PORT:
            ScalerDpRx1Initial(enumDpVersion, enumDpLaneCount, enumDpRxMstPort, enumFreeSyncSupport, enumAudioSupport, enumDpHdrSupport);
            break;
#endif

#if((_D6_INPUT_PORT_TYPE == _D6_DP_PORT) && (_D6_INPUT_PORT_SWITCH_FROM == _SWITCH_FROM_D0))
        case _D6_INPUT_PORT:
            ScalerDpRx6Initial(enumDpVersion, enumDpLaneCount, enumDpRxMstPort, enumFreeSyncSupport, enumAudioSupport, enumDpHdrSupport);
            break;
#endif
...
#if(_DP_HDR10_SUPPORT == _ON)
#if(_DP_HDR_VENDOR_SUPPORT != _DP_HDR_VENDOR_SUPPORT_NONE)
    else if(ScalerGetBit_EXINT(PB7_FB_DUMMY_1, (_BIT3 | _BIT2)) == (_BIT3 | _BIT2))
    {
        BYTE ucDpRxAuxCmdAddrStart = 0;
        BYTE ucDpRxAuxCmdAddrEnd = 0;

        ucDpRxAuxCmdAddrEnd = ScalerGetByte_EXINT(PB7_D4_AUX_RX_ADDR_L) - 1;
        ucDpRxAuxCmdAddrStart = ucDpRxAuxCmdAddrEnd - (ScalerGetByte_EXINT(PB7_D5_AUX_RXLEN) & 0x0F);

        // Fw Reply Defer if aux read request include 00340-00359
        if((ScalerGetBit_EXINT(PB7_D2_AUX_RX_CMD, (_BIT7 | _BIT6 | _BIT5 | _BIT4)) == (_BIT7 | _BIT4)) &&
           ((ucDpRxAuxCmdAddrEnd >= 0x40) && (ucDpRxAuxCmdAddrStart <= 0x59)))
        {
            if(ScalerGetBit_EXINT(PB7_D0_AUX_MODE_SET, _BIT4) == 0x00)
            {
                // Reset Aux FIFO
...
#if(_DP_HDR10_SUPPORT == _ON)
#if(_DP_HDR_VENDOR_SUPPORT != _DP_HDR_VENDOR_SUPPORT_NONE)
    else if(ScalerGetBit_EXINT(PBA_FB_DUMMY_1, (_BIT3 | _BIT2)) == (_BIT3 | _BIT2))
    {
        BYTE ucDpRxAuxCmdAddrStart = 0;
        BYTE ucDpRxAuxCmdAddrEnd = 0;

        ucDpRxAuxCmdAddrEnd = ScalerGetByte_EXINT(PBA_D4_AUX_RX_ADDR_L) - 1;
        ucDpRxAuxCmdAddrStart = ucDpRxAuxCmdAddrEnd - (ScalerGetByte_EXINT(PBA_D5_AUX_RXLEN) & 0x0F);

        // Fw Reply Defer if aux read request include 00340-00359
        if((ScalerGetBit_EXINT(PBA_D2_AUX_RX_CMD, (_BIT7 | _BIT6 | _BIT5 | _BIT4)) == (_BIT7 | _BIT4)) &&
           ((ucDpRxAuxCmdAddrEnd >= 0x40) && (ucDpRxAuxCmdAddrStart <= 0x59)))
        {
            if(ScalerGetBit_EXINT(PBA_D0_AUX_MODE_SET, _BIT4) == 0x00)
            {
...
#endif
#if(_DP_AUX_RX_DPCD_HDR_CAP_WILDCARD_SUPPORT == _ON)
    ...
    switch(ucInputPort)
    {
        ...
        case _D0_INPUT_PORT:
            ...
            if(ScalerDpRxHdrGetSupport(_D0_INPUT_PORT) != enumDpHdrSupport)
            {
                ...
                SET_DP_RX_D0_HDR_CAPABILITY_SWITCH();

                SET_DP_HDR_D0_SUPPORT(enumDpHdrSupport);
            }
        ...
        case _D1_INPUT_PORT:
            ...
            if(ScalerDpRxHdrGetSupport(_D1_INPUT_PORT) != enumDpHdrSupport)
            {
                ...
                SET_DP_RX_D1_HDR_CAPABILITY_SWITCH();

                SET_DP_HDR_D1_SUPPORT(enumDpHdrSupport);
            }
...
EnumDpHdrSupport ScalerDpRxHdrGetSupport(BYTE ucInputPort)
{
    switch(ucInputPort)
    {
        ...
        case _D0_INPUT_PORT:
            ...
            return GET_DP_HDR_D0_SUPPORT();
#endif

#if(_D1_DP_EXIST == _ON)
        case _D1_INPUT_PORT:

            return GET_DP_HDR_D1_SUPPORT();
#endif
...
//--------------------------------------------------
// Description  : Check Aux Ch power detect low should reset dpcd or not for compatibility
// Input Value  : enumInputPort
// Output Value : _TRUE/_FALSE
//--------------------------------------------------
bit ScalerDpRxChPowerDownResetCheck(BYTE ucInputPort)
{
    if((ScalerDpRxGetDpcdBitInfo(ucInputPort, 0x00, 0x06, 0x00, (_BIT2 | _BIT1 | _BIT0)) != _BIT1) &&
       (ScalerDpRxGetDpcdBitInfo(ucInputPort, 0x00, 0x06, 0x00, (_BIT2 | _BIT1 | _BIT0)) != (_BIT2 | _BIT0)))
    {
        if(ScalerDpRxGetLinkTrainingPass(ucInputPort) == _TRUE)
        {
            return _FALSE;
        }
    }
...
bit ScalerDpRxGetLinkTrainingPass(BYTE ucInputPort)
{
    if(ScalerDpRxGetLTStatus(ucInputPort) == _DP_NORMAL_LINK_TRAINING_PASS)
    {
        return _TRUE;
    }

    if(ScalerDpRxGetLTStatus(ucInputPort) == _DP_FAKE_LINK_TRAINING_PASS)
    {
        return _TRUE;
    }
...
BYTE ScalerDpRxGetLTStatus(BYTE ucInputPort)
{
    switch(ucInputPort)
    {
        ...
        case _D0_INPUT_PORT:
            ...
            return GET_DP_RX0_LINK_TRAINING_STATUS();
#endif

#if(_D1_DP_EXIST == _ON)
        case _D1_INPUT_PORT:

            return GET_DP_RX1_LINK_TRAINING_STATUS();
#endif
