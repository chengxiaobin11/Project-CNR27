...
void ScalerDpRxMstDownRequestReceive(void);
void ScalerDpRxMstDownReplySend(void);
void ScalerDpRxMstDownRequestAnal(void);
void ScalerDpRxMstDownReplyAnal(void);

#if(_DP_TX_SUPPORT == _ON)
void ScalerDpRxMstUpRequestSend(void);
void ScalerDpRxMstUpReplyReceive(void);
...
void ScalerDpRxMstIntHandle_EXINT0(void) using 1
{
       ...
       (GET_REG_DP_RX_111_IRQ_FLAG() == _TRUE))
    {
        ...
        if(((GET_REG_DP_DPCD_INFO_EXINT0(0x00, 0x00, 0x21) & _BIT0) == _BIT0) &&
           ((GET_REG_DP_DPCD_INFO_EXINT0(0x00, 0x01, 0x11) & _BIT0) == _BIT0))
        {
            // Enable MST
...
void ScalerDpRxMstDownRequestAnal(void)
{
    if(GET_DP_RX_DOWN_REQUEST_ANAL() == _TRUE)
    {
        if(ScalerDpRxMstCheckMsgCRC(_DP_DOWN_REQUEST_MSG) == _TRUE)
        {
            ...
            {
                EnumSourceSearchPort enumInputPort = ScalerDpRxMstGetInputPort(g_enumDpMSTCapablePort);
                ...
                // Let "Power Status Write Event" Keep Debouncing While Receiving MSGs to Stay in Fake PS or Keep The Source Detecting
                if(ScalerSyncDpRxGetSourceWritePowerStatusEvent(enumInputPort) == _DP_SOURCE_WRITE_POWER_STATUS_EVENT_TIMER_ACTIVE)
                {
                    ScalerSyncDpRxSourceWritePowerStatusEventTimerReactive(enumInputPort);
                }
            ...
            switch(GET_DP_DOWN_REQUEST_REMOTE_I2C_WRITE_MSG_WRITE_DEVICE_ID())
            {
                ...
                case _DP_MSG_IIC_DEVICE_DDCCI:
                    ...
                    if(pData[1] <= _DDCCI_RXBUF_LENGTH)
                    {
                        ...
                        {
#if(_DDCCI_RXBUF_LENGTH > _DP_MST_RX_REQUEST_REMOTE_I2C_WRITE_MSG_DATA_TO_WRITE_MAX_LEN)
                            if(pData[0] >= _DP_MST_RX_REQUEST_REMOTE_I2C_WRITE_MSG_DATA_TO_WRITE_MAX_LEN)
                            {
                                g_pucDdcciRxBuf[pData[0]] = 0;
                            }
                            else
#endif
                            {
                        ...
                        SET_RUN_DDCCI_COMMAND_SRC(_DDCCI_CMD_SRC_MST);
                    }
...
void ScalerDpRxMstCableUnplugReset(BYTE ucInputPort)
{
    ...
    ScalerSyncDpRxClrSourceWritePowerStatusEvent(ucInputPort);
}
...
bit ScalerDpRxMstGetSrcMsgEnable(EnumSourceSearchPort enumInputPort)
{
    if(ScalerDpRxGetDpcdBitInfo(enumInputPort, 0x00, 0x00, 0x21, _BIT0) == _BIT0)
    {
        if(ScalerDpRxGetDpcdBitInfo(enumInputPort, 0x00, 0x01, 0x11, _BIT1) == _BIT1)
        {
            return _ENABLE;
        }
    }
...
//--------------------------------------------------
// Description  : ScalerDpRxMstDownRequestProc
// Input Value  : None
// Output Value : None
//--------------------------------------------------
void ScalerDpRxMstDownRequestProc(void)
{
    ScalerDpRxMstDownRequestReceive();
...
//--------------------------------------------------
// Description  : ScalerDpRxMstDownReplytProc
// Input Value  : None
// Output Value : None
//--------------------------------------------------
void ScalerDpRxMstDownReplyProc(void)
{
    ...
    ScalerDpRxMstDownReplySend();
}

//--------------------------------------------------
// Description  : ScalerDpRxMstUpRequestProc
// Input Value  : None
// Output Value : None
//--------------------------------------------------
void ScalerDpRxMstUpRequestProc(void)
{
    ...
    ScalerDpRxMstUpRequestSend();
}

//--------------------------------------------------
// Description  : ScalerDpRxMstUpReplyProc
// Input Value  : None
// Output Value : None
//--------------------------------------------------
void ScalerDpRxMstUpReplyProc(void)
{
    ScalerDpRxMstUpReplyReceive();
