...
bit ScalerDpTxDfpDeviceFreesyncEnable(void);
void ScalerDpTxDfpDeviceFreesyncEnableReset(void);
#endif
//****************************************************************************
...
#if(_DP_FREESYNC_SUPPORT == _ON)
    if((pucData[7] & _BIT6) == _BIT6)
    {
        SET_DP_TX_DOWNSTREAM_MSA_IGNORED(_TRUE);
    }
    else
    {
        SET_DP_TX_DOWNSTREAM_MSA_IGNORED(_FALSE);
    }
#endif
...
#if(_DP_FREESYNC_SUPPORT == _ON)
                ScalerDpTxDfpDeviceFreesyncEnableReset();
#endif
...
#if(_DP_FREESYNC_SUPPORT == _ON)
                if(ScalerDpTxDfpDeviceFreesyncEnable() == _TRUE)
                {
                    DebugMessageMst("Enable Down Device Freesync", 1);
                }
#endif
                // Setting DP TX MAC
                ScalerDpTxSSTSetting();
...
#if(_DP_FREESYNC_SUPPORT == _ON)
                if((GET_DP_FREESYNC_ENABLED() == _TRUE) && (ucPortType == _PORT_DP))
                {
                    // Bypass DP Tx Hw Measure because Dp Drr Deonly mode constraint
                    ScalerSetBit(PBB_DF_VESA_FMT_REGEN, ~_BIT1, _BIT1);
                }
                else
                {
                    // Select from Measure block's output
                    ScalerSetBit(PBB_DF_VESA_FMT_REGEN, ~_BIT1, 0x00);
                }
#endif
...
bit ScalerDpTxDfpDeviceFreesyncEnable(void)
{
    ...
    if(GET_DP_TX_DOWNSTREAM_MSA_IGNORED() == _TRUE)
    {
        ...
        if(ScalerDpTxAuxNativeRead(0x00, 0x01, 0x07, 1, &ucFreeSyncConfig) == _TRUE)
        {
            bEnable = GET_DP_FREESYNC_ENABLED();

            if((ucFreeSyncConfig & _BIT7) != ((BYTE)bEnable << 7))
            {
                ucFreeSyncConfig = ((ucFreeSyncConfig & ~_BIT7) | ((BYTE)bEnable << 7));

                if(ScalerDpTxAuxNativeWrite(0x00, 0x01, 0x07, 1, &ucFreeSyncConfig) == _TRUE)
                {
                    return bEnable;
                }
            }
...
//--------------------------------------------------
// Description  : DP Tx Down Device FreeSync Reset
// Input Value  : None
// Output Value : None
//--------------------------------------------------
void ScalerDpTxDfpDeviceFreesyncEnableReset(void)
{
    ...
    if(ScalerDpTxAuxNativeRead(0x00, 0x01, 0x07, 1, &ucFreeSyncConfig) == _TRUE)
    {
        if((ucFreeSyncConfig & _BIT7) == _BIT7)
        {
            ucFreeSyncConfig &= ~_BIT7;

            // Disable DFP Freesync Enable
            ScalerDpTxAuxNativeWrite(0x00, 0x01, 0x07, 1, &ucFreeSyncConfig);
        }
    }
}
#endif
