...
#if((_TYPE_C_EMB_PD3_1_EPR_SUPPORT == _ON) && (_TYPE_C_PMIC_VBUS_MAX_VOLTAGE != 3200))
#warning "NOTE: Type-C can only support EPR Mode 28V application, so '_TYPE_C_PMIC_VBUS_MAX_VOLTAGE' should be 3200(Unit: 10mV) and the Vbus Voltage Divider of PCB Circuit should be 16!"
#endif
#endif

#if(_TYPE_C_EMB_PD_VERSION >= _PD_VERSION_3_2)
#warning "NOTE: Type-C Power Source shall support SPR AVS PDO if PD Power > 27W when '_TYPE_C_EMB_PD_VERSION' >= '_PD_VERSION_3_2'. The Voltage Step Size of SPR AVS PDO is 100mV, applicable PMICs include '_PMIC_RAA489800', '_PMIC_SY9228', '_PMIC_MP4247', '_PMIC_NCP81239'!"
#endif
...
WORD ScalerTypeCPowerGetAdcOutputVoltage(BYTE ucInputPort);
WORD ScalerTypeCPowerGetAdcOutputCurrent(BYTE ucInputPort);
...
WORD ScalerTypeCPowerRx0CorrectionToRawData(WORD usIdealVoltage)
{
    ...
    // Transition Voltage to ADC input domain (/ 100 (Transition 10mV to V) / 2 (10Bit ADC Channel Max Allowable Voltage))
    ulCorrectionCompThr = ulCorrectionCompThr * 100 / (_TYPE_C_PMIC_VBUS_MAX_VOLTAGE / 200);
...
void ScalerTypeCPowerRx0UnattachVthAdjust(void)
{
    ...
    WORD usVoltage = ScalerTypeCPowerRx0CorrectionToRawData(22);
...
void ScalerTypeCPowerRx0UnattachVthAdjust_EXINT0(WORD usVoltage) using 1
{
    ...
    if(usVoltage == 500)
    {
        ...
        // TD 4.5.4 : Adjust Threshold to ((3.67V + 0.8V) / 2 = 2.24V) Lower than 3.67V to Cover Unexpected Unattach Caused by Tester Vbus Undershoot and IC Variation in ADC Detection. (MM1AE-4460)
        usVoltage = ScalerTypeCPowerRx0CorrectionToRawData_EXINT0(22);
    }
    else
    {
...
//--------------------------------------------------
// Description  : Return Embedded Port Ctrl ADC Channel Output Voltage Value
...
WORD ScalerTypeCPowerGetAdcOutputVoltage(BYTE ucInputPort)
{
    ...
    switch(ucInputPort)
    {
        ...
        case _D0_INPUT_PORT:
            ...
            usAdcOutputVoltage = ((WORD)((((DWORD)ScalerTypeCPowerRx0CorrectionToIdealData(ScalerTypeCPowerGetAdcOutput(_TYPE_C_UFP_VMON))) * _TYPE_C_PMIC_VBUS_MAX_VOLTAGE) / 1024));
...
//--------------------------------------------------
// Description  : Return Embedded Port Ctrl ADC Channel Output Current Value
// Input Value  : ucInputPort
...
WORD ScalerTypeCPowerGetAdcOutputCurrent(BYTE ucInputPort)
{
    ...
    switch(ucInputPort)
    {
        ...
        case _D0_INPUT_PORT:
            ...
            usAdcOutputCurrent = ((WORD)((((DWORD)ScalerTypeCPowerRx0CorrectionToIdealData(ScalerTypeCPowerGetAdcOutput(_TYPE_C_UFP_IMON))) * _TYPE_C_PMIC_IOUT_MAX_CURRENT) / 1024));
