...
//--------------------------------------------------
#define _TYPE_C_WD_EVENT_TIME               250    // unit: ms
...
// Timer Control Macro
#define GET_PMIC_WD_TIMER_VALUE()           (g_usWDTimer)
#define SET_PMIC_WD_TIMER_VALUE(x)          (g_usWDTimer = (x))
#define CLR_PMIC_WD_TIMER_VALUE()           (g_usWDTimer = 0)

#define GET_PMIC_PEAK_CURRENT_TYPE()        (g_enumTypeCPDPeakCurType)
#define SET_PMIC_PEAK_CURRENT_TYPE(x)       (g_enumTypeCPDPeakCurType = (x))

#define GET_PMIC_PEAK_CURRENT_PROTECT()     (g_enumTypeCPDPeakCurProtect)
#define SET_PMIC_PEAK_CURRENT_PROTECT(x)    (g_enumTypeCPDPeakCurProtect = (x))
...
EnumTypeCPDPeakCurProtect g_enumTypeCPDPeakCurProtect;
...
#if(((_TYPE_C_PMIC_TYPE == _PMIC_RAA489800) && (_TYPE_C_EMB_PD3_PPS_SUPPORT == _ON)) || (_TYPE_C_PMIC_TYPE == _PMIC_SY8665) || (_TYPE_C_PMIC_TYPE == _PMIC_SY9228) || (_TYPE_C_PMIC_TYPE == _PMIC_SY9329))
...
bit ScalerTypeCPmicRx0Setting(bit bAction, bit bPowerDirection, EnumPDPdoType enumPdoType, WORD usVoltage, WORD usCurrent, EnumPDPeakCurType enumPeakCurrent)
{
    ...
    if((bAction == _ON) && (bPowerDirection == _PD_POWER_SRC))
    {
        ...
        if((GET_PMIC_PEAK_CURRENT_PROTECT() == _TYPE_C_PEAK_CURRENT_PROTECT_ENABLE) && (enumPeakCurrent != _PD_PEAK_CUR_NONE))
        {
...
void ScalerTypeCPmicRx0FwOcpIntHandler_EXINT0(void) using 1
{
    ...
    else
    {
        ...
        if(usOcpDelay < _TYPE_C_WD_EVENT_TIME)
        {
            // Set WD Timer Event Time
            SET_PMIC_WD_TIMER_VALUE(usOcpDelay);
        ...
        {
            // Set WD Timer Event Time
            SET_PMIC_WD_TIMER_VALUE(_TYPE_C_WD_EVENT_TIME);

            // Active WD Timer Event For Pmic Shutdown Delay
            ScalerTimerWDActivateTimerEvent_EXINT0(_TYPE_C_WD_EVENT_TIME, _SCALER_WD_TIMER_EVENT_PMIC_RX0_FW_OCP_DELAY);
        }
    }
}
...
// Input Value  : enumEventID     : _SCALER_WD_TIMER_EVENT_PMIC_RX0_FW_OCP_DELAY
//                pucActiveWDID   : _SCALER_WD_TIMER_EVENT_PMIC_RX0_FW_OCP_DELAY
...
{
    WORD usOcpDelay = GET_PMIC_FW_OCP_DELAY(GET_PMIC_FW_OCP_PROTECT_LEVEL());

    switch(enumEventID)
    {
        case _SCALER_WD_TIMER_EVENT_PMIC_RX0_FW_OCP_DELAY:

            if(GET_PMIC_WD_TIMER_VALUE() < usOcpDelay)
            {
                if((usOcpDelay - GET_PMIC_WD_TIMER_VALUE()) > _TYPE_C_WD_EVENT_TIME)
                {
                    *pucActiveWDTime = _TYPE_C_WD_EVENT_TIME;
                    *pucActiveWDID = _SCALER_WD_TIMER_EVENT_PMIC_RX0_FW_OCP_DELAY;

                    SET_PMIC_WD_TIMER_VALUE(GET_PMIC_WD_TIMER_VALUE() + _TYPE_C_WD_EVENT_TIME);
                }
                else
                {
                    *pucActiveWDTime = usOcpDelay - GET_PMIC_WD_TIMER_VALUE();
                    *pucActiveWDID = _SCALER_WD_TIMER_EVENT_PMIC_RX0_FW_OCP_DELAY;
