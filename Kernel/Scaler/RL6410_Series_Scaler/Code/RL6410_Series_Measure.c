...
void ScalerMeasureCheckFirstLineShift(EnumSourceSearchPort enumSourceSearchPort, WORD usFrameHwidth)
{
    ...
    BYTE ucDiffPixel = (ScalerGetBit(P30_00_SYNC_SELECT, _BIT7) == _BIT7) ? 2 : 1;
       ...
       (ScalerGetBit((usHdmiPage + P71_A4_TMDS_CTRL), (_BIT4 | _BIT3)) == (_BIT4 | _BIT3)))
    {
        ...
        PDATA_WORD(2) = (PDATA_WORD(1) - PDATA_WORD(0) + 1) * ucHwidthFactor; // H width
...
{
#if((_DVI_SUPPORT == _ON) || (_HDMI_SUPPORT == _ON))
    EnumDeOnlyConfig enumDeOnlyConfig = ScalerTMDSGetDeOnlyModeConfig(enumSourceSearchPort);
#endif
