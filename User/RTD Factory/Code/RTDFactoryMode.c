...
void RTDFactoryMainMenu(void)
{
    ...
    RTDFactoryOsdFuncClearOsdFont(0, 0, _FACTORY_OSD_COL, _FACTORY_OSD_ROW, 0x00);
        ...
        {
            FactoryEDIDInfo(_DDCRAM_VGA,_ManValue_VGA,((_A0_INPUT_PORT_TYPE)&0x0F));
			DebugMessageOsd("3.------------",_DDCRAM_VGA[0x08]);
			DebugMessageOsd("3._DDCRAM_VGA",_DDCRAM_VGA[0x08]);
			DebugMessageOsd("3._DDCRAM_VGA",_DDCRAM_VGA[0x09]);
        }
    #endif
    //D0 PORT      
        ...
        {
           FactoryEDIDInfo(_DDCRAM_D0,_ManValue_D0,((_D0_INPUT_PORT_TYPE)&0x0F));
        }
    #endif
    //D1 PORT
       ...
       // if(GET_OSD_INPUT_PORT_OSD_ITEM==_OSD_INPUT_D1)
        {
            FactoryEDIDInfo(_DDCRAM_D1,_ManValue_D1,((_D1_INPUT_PORT_TYPE)&0x0F));
        }
    #endif
    //D2 PORT
        ...
        {
            FactoryEDIDInfo(_DDCRAM_D2,_ManValue_D2,((_D2_INPUT_PORT_TYPE)&0x0F));
        }
    #endif
    //D3 PORT
    #if(_D3_EMBEDDED_DDCRAM_MAX_SIZE != _EDID_SIZE_NONE) 
        //if(GET_OSD_INPUT_PORT_OSD_ITEM==_OSD_INPUT_D3)
        {
            FactoryEDIDInfo(_DDCRAM_D3,_ManValue_D3,((_D3_INPUT_PORT_TYPE)&0x0F));
        }
    #endif
#endif
    ...
    RTDFactoryOsdFuncPutStringProp(ROW(_FACTORY_MODEL_NAME_ROW), COL(_FACTORY_FUNCTION_COL), _PFONT_FACTORY_PAGE_0, tFONT_FACTORY_EUROPE, tOSD_FACTORY_PAGE_TABLE[0], _ENGLISH);
		...
		RTDFactoryOsdFuncPutStringProp(ROW(_FACTORY_PANEL_NAME_ROW), COL(_FACTORY_FUNCTION_COL + 13), _PFONT_FACTORY_PAGE_0, tFONT_FACTORY_EUROPE, tOSD_FACTORY_PAGE_TABLE[1], _ENGLISH);
	#else
		RTDFactoryOsdFuncPutStringProp(ROW(_FACTORY_PANEL_NAME_ROW), COL(_FACTORY_FUNCTION_COL + 11), _PFONT_FACTORY_PAGE_0, tFONT_FACTORY_EUROPE, tOSD_FACTORY_PAGE_TABLE[1], _ENGLISH);
	#endif
  ...
  //  RTDFactoryOsdFuncPutStringProp(ROW(_FACTORY_OSD_PAGE_ROW), COL(_FACTORY_FUNCTION_COL), _PFONT_FACTORY_PAGE_0, tFONT_FACTORY_EUROPE, tOSD_FACTORY_PAGE_TABLE[18], _ENGLISH);
	...
	RTDFactoryOsdFuncShowChecksum(ROW(_FACTORY_EXIT_ROW+1), COL(20), (_ALIGN_LEFT | _FORCE_SHOW_NUMBER_OFF | _SHOW_5), _PFONT_FACTORY_PAGE_0);
#if(_DEF_IR_SHOW)
	RTDFactoryOsdFuncShowHexNumber(ROW(_FACTORY_EXIT_ROW+2), COL(1),  _PFONT_FACTORY_PAGE_0);
#endif
...
#if(Project_ID == ID_TIEMU_HK2513E02_1V1H__SGM2701B01_2_FHD75HZ__20200718)||(ID_TIEMU_HK2513E02_1V1H__M270HVR01_1_FHD75HZ__20200908 == Project_ID)
