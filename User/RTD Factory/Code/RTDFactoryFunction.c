...
//#warning "zhe is TEST!!"
//#define _MAIN_PAGE_MODEL_NAME                 _M__,_o__,_d__,_e__,_l__,__,_COLON__,__,_2__,_5__,_5__,_6__

//#undef _BURNIN_STR_IC_NAME
//#warning "zhe is TEST!!"
//#define _BURNIN_STR_IC_NAME                 _M__,_o__,_d__,_e__,_l__,__,_COLON__,__,_2__,_5__,_5__,_6__
...
#if(Project_ID == ID_TJ_HK2795E051DP2HDMI__AUO_M270KCJ_K7B_QHD165HZ__20200917)
#define _FACTORY_COLORTEMP_TEXT4            _5__,_6__,_0__,_0__
#else
...
{
     _C__,_H__,_K__,_S__, _U__,_M__,_END_,
};
...
void RTDFactoryOsdFuncPutStringProp(BYTE ucRow, BYTE ucCol, BYTE ucFptsSelect, BYTE *pucFontTable, BYTE *pucArray, BYTE ucLanguage)
{
    ...
    while(*(pucArray + pData[0]) != _END_)
    {
        ...
        if((ucFptsSelect >= _PFONT_FACTORY_PAGE_ITEM_1) && (ucFptsSelect < _PFONT_FACTORY_PAGE_ITEM_END)||(ucFptsSelect == _PFONT_FACTORY_PAGE_1))
        {
...
*/
    ...
    else if((ucFptsSelect >= _PFONT_FACTORY_PAGE_ITEM_1) && (ucFptsSelect <= _PFONT_FACTORY_PAGE_ITEM_END))
    {
...
#if(Project_ID == ID_DATONG_HK2513E02_1V1H__238FHD75_AXMT248__20201024)||(Project_ID == ID_DATONG_HK25561DP1HDMI__AUO_M270R01_2_FHD165HZ__202001204)||(Project_ID == ID_DATONG_HK25561DP1HDMI__AUO_M270R01_2_FHD165HZ__202001204)||(Project_ID == ID_DATONG_HK25561DP1HDMI__AUO_M215HTN03_0_FHD165HZ__202001026)
				
	bLED1 = ~bLED1;

	bLED2 = ~bLED2;
#elif _DEF_DX_KEY_LED
	bLED1 = ~bLED1;
...
void GetEDIDPanelSize(BYTE *pDDCRAMData)
{
	...
	DWORD xdata  xx;
    BYTE xdata H_Low8,V_Low8,H_high4,V_high4;
...
void GetOSDEDIDResult(BYTE *pDDCRAMData,BYTE PortType)
{
	...
	{
		if ((pDDCRAMData[127] == CheckSum)&&(pDDCRAMData[0x7E]==0)/*&&((pDDCRAMData[0x14]&0x80)==0)*/)//&&(( _PANEL_DH_WIDTH == Hactive||(_PANEL_DH_WIDTH == (Hactive+6))) && ( _PANEL_DV_HEIGHT== Vactive)))
		{
			CheckSum_ok=1;
		}
		else
	...
	DebugMessageOsd("3.1",pDDCRAMData[127]);
	DebugMessageOsd("3.1",CheckSum);
		if (((pDDCRAMData[127] == CheckSum && pDDCRAMData[126] == 0x01)||(pDDCRAMData[127] == (CheckSum) && pDDCRAMData[126] == 0x02))/*&&((pDDCRAMData[0x14]&0x80)==0x80)&&(( _PANEL_DH_WIDTH == Hactive||(_PANEL_DH_WIDTH == (Hactive+6))) && ( _PANEL_DV_HEIGHT== Vactive))*/)
		{
			DebugMessageOsd("3.2",PortType);
			DebugMessageOsd("3.2",pDDCRAMData[0x13]);
			
			if(((PortType==_PORT_HDMI)||(PortType==_PORT_HDMI20)) && pDDCRAMData[0x13]==0x3)
				CheckSum_ok=1;
			else if((PortType==_PORT_DP) && pDDCRAMData[0x13]==0x4)
				CheckSum_ok=1;
			else
				CheckSum_ok=0;
			
			//DebugMessageOsd("3.3",CheckSum_ok);
		}
		#if(_DEF_GAME_MODE)
	...
	else
	{
		...
		pData[2]=_END_;
		RTDFactoryOsdFuncPutStringProp(ROW(_BURNIN_EDID0_PORT_ROW+(cnt*EDID_ROW_NUM)), COL(_BURNIN_EDID0_PORT_STA_COL), _PFONT_FACTORY_PAGE_0, tFONT_FACTORY_EUROPE, pData, _ENGLISH);
        RTDFactoryOsdFuncChangeColor1Bit(ROW(_BURNIN_EDID0_PORT_ROW+(cnt*EDID_ROW_NUM)), _BURNIN_EDID0_PORT_STA_COL, 4, 1, COLOR(_CP_RED, _CP_BG));
	}
}
//--------------------------------------------------
// Description  :
...
#if(_DEF_FACTORY_EDIDINFO)||_DEF_BURN_EDID2
void GetOSDEDID_STR1(BYTE *pDDCRAMData,BYTE index)
{
	BYTE i,ok,lenght=5;
	BYTE xdata pData[16];

	if(index==1 || index==2)//sn / NAME
	{
		...
		pData[4] = 0;
		pData[5] = _END_;
		for(i=0x40,ok=0;i<0X80;i++) 
		{
		
			if((*(pDDCRAMData + i) != pData[ok])) 
			{
				ok = 0;
				continue;
			}
		...
		DebugMessageOsd("3.edid str 0",i);
		if(i>0x80)
			return ;
		DebugMessageOsd("3.edid str 1",1);
		if(index==1)//SN
		{
			pData[0] = _S__;
			pData[1] = _N__;
			pData[2] = _COLON__;
			pData[3] = _END_;
			RTDFactoryOsdFuncPutStringProp(ROW(_BURNIN_EDID0_SN_ROW+(cnt*EDID_ROW_NUM)), COL(_BURNIN_MSG_COL), _PFONT_FACTORY_PAGE_1, tFONT_FACTORY_EUROPE, pData, _ENGLISH);
			DebugMessageOsd("3.edid str 1",2);
		}
		else if(index==2)//NAME
		{
			...
			pData[4] = _COLON__;
			pData[5] = _END_;
			RTDFactoryOsdFuncPutStringProp(ROW(_BURNIN_EDID0_NAME_ROW+(cnt*EDID_ROW_NUM)), COL(_BURNIN_MSG_COL), _PFONT_FACTORY_PAGE_1, tFONT_FACTORY_EUROPE, pData, _ENGLISH);
			DebugMessageOsd("3.edid str 1",3);
		}
		ok=i;
		
		for(i=0;i<13;i++)
		{
			if(pDDCRAMData[ok+i]>=0x30&&pDDCRAMData[ok+i]<=0x39)
				pData[i]=pDDCRAMData[ok+i]-0x30+_0__;
			else if(pDDCRAMData[ok+i]>=0x41&&pDDCRAMData[ok+i]<=0x5A)
				pData[i]=pDDCRAMData[ok+i]-0x41+_A__;		
			else if(pDDCRAMData[ok+i]>=0x61&&pDDCRAMData[ok+i]<=0x7A)
				pData[i]=pDDCRAMData[ok+i]-0x61+_a__;		
			else if(pDDCRAMData[ok+i]==0x20||pDDCRAMData[ok+i]==0x0A)
				pData[i]=__;	
			else if(pDDCRAMData[ok+i]==0x2F)
				pData[i]=_AND__;		
		
		
		DebugMessageOsd("3.edid str 40",(ok+i));
		DebugMessageOsd("3.edid str 41",pDDCRAMData[ok+i]);
		DebugMessageOsd("3.edid str 42",pData[i]);
		}
		
		pData[13]=_END_;		
		if(index==1)//SN
			RTDFactoryOsdFuncPutStringProp(ROW(_BURNIN_EDID0_SN_ROW+(cnt*EDID_ROW_NUM)), COL(_BURNIN_SN_COL), _PFONT_FACTORY_PAGE_1, tFONT_FACTORY_EUROPE, pData, _ENGLISH);
		else if(index==2)//NAME
			RTDFactoryOsdFuncPutStringProp(ROW(_BURNIN_EDID0_NAME_ROW+(cnt*EDID_ROW_NUM)), COL(_BURNIN_NAME_COL), _PFONT_FACTORY_PAGE_1, tFONT_FACTORY_EUROPE, pData, _ENGLISH);
		
		DebugMessageOsd("3.edid str 1",4);
	}
...
BYTE code tsIN_TYPEC[] =
{
    _T__,_Y__,_P__,_E__,_C__,_END_,
};


BYTE code tsIN_LINUX[] =
{
    _L__,_I__,_N__,_U__,_X__,_END_,
};

code BYTE *tIN_SOURCE_TYPE_STATUS[] =
{
	tsIN_TYPEC,tsIN_HDMI,tsIN_LINUX,
};
...
#elif(_PCB_TYPE == _RL6432__156PIN__HKMRT2557E17_1T1P1H_LVDS)
BYTE code tsIN_TYPEC[] =
{
    _T__,_Y__,_P__,_E__,_C__,_END_,
};
...
#elif(_PCB_TYPE == _RL6432__156PIN__HKMRT2556E01__2H1DP__LVDS)
code BYTE *tIN_SOURCE_TYPE_STATUS[] =
{
#if(_SOURCE_AUTO_ON)
		tsIN_AUTO,
...
#elif(_PCB_TYPE == _RL6463__128PIN__HKMRT2525E02__1A2H__LVDS)
code BYTE *tIN_SOURCE_TYPE_STATUS[] =
{
#if(_SOURCE_AUTO_ON)
		tsIN_AUTO,
...
//#include "RTDOSDTABLEINPUT.h"

//OsdPropGetStringTableAddress
...
{
	//EDID Port name
	 RTDFactoryOsdFuncPutStringProp(ROW(_BURNIN_EDID0_PORT_ROW+(cnt*EDID_ROW_NUM)), COL(_BURNIN_EDID0_PORT_COL), _PFONT_FACTORY_PAGE_0, tFONT_FACTORY_EUROPE, tIN_SOURCE_TYPE_STATUS[PortNameInedx], _ENGLISH);
 ...
 #if _DEF_BURN_EDID2
     GetOSDEDID_STR1(pDDCRAMData,1);//SN
 	 GetOSDEDID_STR1(pDDCRAMData,2);//NAME
 #endif
...
void RTDBurnInGetEdidInfo(BYTE Port)
{
	...
	switch(Port)
	{
	    ...
	    case _OSD_INPUT_D4:
				...
				DrawEDIDInfo(_DDCRAM_D4,_ManValue_D4,_OSD_INPUT_D4,((_D4_INPUT_PORT_TYPE)&0x0F));
				...
				ManVal=((pucBuffer[_EDID_MAN_NAME_INDEX0]<<8)|pucBuffer[_EDID_MAN_NAME_INDEX1]);
				DrawEDIDInfo(pucBuffer,ManVal,_OSD_INPUT_D4,((_D4_INPUT_PORT_TYPE)&0x0F));
			#endif
	        break;
	#endif
	    ...
	    case _OSD_INPUT_D5:
				...
				DrawEDIDInfo(_DDCRAM_D5,_ManValue_D5,_OSD_INPUT_D5,((_D5_INPUT_PORT_TYPE)&0x0F));
				...
				ManVal=((pucBuffer[_EDID_MAN_NAME_INDEX0]<<8)|pucBuffer[_EDID_MAN_NAME_INDEX1]);
				DrawEDIDInfo(pucBuffer,ManVal,_OSD_INPUT_D5,((_D5_INPUT_PORT_TYPE)&0x0F));
			#endif
	        break;
	#endif
...
void RTDBurnInEdidMenu(void)
{
	BYTE Port=0;
	cnt=0;//must set to 0
#if(Project_ID == ID_DX_HK2556E01_1P2H_SG2361B023_FHD60HZ_20240118)
	...
	RTDBurnInGetEdidInfo(_OSD_INPUT_D0);
#endif
#elif 0//(_PCB_TYPE == _RL6432__156PIN__HKMRT2557E17_1T1P1H_LVDS)
	RTDBurnInGetEdidInfo(_OSD_INPUT_D0);
	RTDBurnInGetEdidInfo(_OSD_INPUT_D1);
	//RTDBurnInGetEdidInfo(_OSD_INPUT_D3);
#else
...
void RTDFactoryOsdFuncShowHexNumber(BYTE ucRow, BYTE ucCol, BYTE ucFontPoint)
{
	BYTE i=0,sum=0,ir_add=0;
    BYTE pucDataSN[32] = {0};
	ir_add=3+2;//IR:0X
	sum+=ir_add;
	sum+=4*3;//ir_show_data
	//0EF1FD02
	pucDataSN[0]=_I__;
	pucDataSN[1]=_R__;
	pucDataSN[2]=_COLON__;
	pucDataSN[3]=_0__;
	pucDataSN[4]=_X__;
	for(i=0;i<4;i++)
	{
		pucDataSN[ir_add+i*3]=_0__+(ir_show_data[i]>>4);
		pucDataSN[ir_add+i*3+1]=_0__+(ir_show_data[i]&0XF);
		pucDataSN[ir_add+i*3+1]=_0__+(ir_show_data[i]&0XF);
	}
	pucDataSN[sum+1]=_END_;
