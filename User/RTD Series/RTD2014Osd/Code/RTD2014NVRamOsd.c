...
#if(Project_ID == ID_SHUNWEI_HK2556E01_1P2H_AUO_M250HAN01_8_LVDS_FHD165HZ_20211117)
#define _CHECKSUM                                   0x12
...
#endif
code char OsdDataFlag[] = {"This is OsdDefaultData Flag V001!"};
...
{
#if(_DEF_OSD_BACKLIGHT)
	_DEF_OSD_BACKLIGHT,
#else
    _DEF_BACKLIGHT,           // ucBackLight;    
#endif
	//--------------------------------------------------------
	_DEF_LANGUAGE,			  // b4Language;
	_DEF_LANGUAGE,			  // b4Language_init
	_DEF_COLORTEMP, 		   // b4ColorTempType;
	//--------------------------------------------------------
    _DEF_H_POS,                       // ucOsdHPos;
    _DEF_V_POS,                       // ucOsdVPos;
    _DEF_OSD_TIMEOUT,                 // ucOsdTimeout;
...
//--------------------------------------------------------
    ...
    _DEF_OSDROTATE,           // b1OsdRotate;
    _OSD_ROTATE_DISPLAY_KEEP_SRC_ASPECT_RATIO,     // b2DispRotationSizeType;
    _OFF,                      // b1OverScan;
//--------------------------------------------------------
    _DEF_GAMMA,               // b3Gamma;
    _DEF_AspectRatio,   // b3AspectRatio;
    _DEF_DDCCIStatus,                      // b1DDCCIStatus;
    _DEF_DCR,                 // b1OsdESStatus;
//--------------------------------------------------------
    _DEF_OD_GAIN,          // ucODGain;
    _DEFAULT_HUE,             // cHue;
...
//--------------------------------------------------------
    ...
    _OFF,                      // b1DpAdaptiveSyncStatus : 1;
		...
		_OSD_CLONE_ON,
#elif(_PCB_TYPE == _RL6432__216PIN__HKM_RT2785E17_1T1P2H_LVDS)
		_OSD_CLONE_ON,
#else
...
// --------------------------------------------------------
	...
	0,	   			   		   // BYTE ucLowBlue
	_AutoPowerOff_OFF,	       // BYTE ucOSDAutoPowerOff 
	...
	_DEF_MPRT,//b1MPRT
	_DEF_OVERCLOCK,  //b1OverClock
	_DEF_DCC,//bDCC
#if(_LED_EFFECT)
    _DEF_OSD_LEDTYPE,//b2OSDPreLedType
#endif

#if(_DEF_MENU_LOCK)
    _OFF,//b1OSDmenuLockType
#endif
#if _DEF_Energy_Msg
	_ON,//BYTE b1EnergyMsg: 1;
#else
	_OFF,//BYTE b1EnergyMsg: 1;
#endif
#if 1//(_OSD_POWER_SAVING_MODE)
	#if(_MC_EUROPE)
	_ON,//b1OSDPowerSaving
	#else
	_OFF,//b1OSDPowerSaving
	#endif
#endif
#if(_DYNAMIC_COLOR_AUTO_MEASURE == _ON)
    0,//BYTE b3OsdDialPointStyle:1;
#endif
...
BYTE code OsdStateDefineDefault[6][6]={     
...
void RTDFlashStartup(void)
{
    ...
    RTDFlashInitialAddr();
#if !_DEF_OSD_2525
	memcpy(OsdStateDefine1,OsdStateDefineDefault,sizeof(OsdStateDefineDefault));
#endif
...
#if _DEBUG_MESSAGE_OSD
			DebugMessageOsd("1.111",11);
...
void RTDFlashRestoreOSDData(void)
{
    BYTE ucItemOffset = 0;
#if _HDMI_24C02_jiaqiang
	BYTE ucEdidFlag = g_stOSDUserData.ucEdidFlag;
#endif
    g_stOSDUserData = g_stOSDDefaultData;
#if _HDMI_24C02_jiaqiang
	BYTE g_stOSDUserData.ucEdidFlag = ucEdidFlag;
#endif
...
{
#if 1
		g_stBriConData = tBriConDefaultData[0];

		if(UserCommonFlashLoadItem(_FLASH_ITEMID_BRICON_SRC0_VGA + 0, pData, 1) == _TRUE)
		{
			RTDFlashSaveBriCon(0);
		}
#else
