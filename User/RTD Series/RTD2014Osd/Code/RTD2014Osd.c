...
//	ScalerDpRx0SetDpLaneMapping(0);
	...
	||(_PCB_TYPE == _RL6449__216PIN__HK_M_RTD2795E06__1T1DP1H__eDPVB1))
					...
					if(GET_FUN_KVM() == _OFF)//自动   TypeC_Proc
					{
						if(SysSourceGetInputPort()== _D0_INPUT_PORT )
						  {
								HUB_SWITCH(0);
...
void MENU_FUN_NULL(void)//放了一些暂时不用的函数，又不想动kernel部分，防止编译器报警告
{
	BYTE test = 0;
...
code void (*OperationTable[])(void) =
{
	...
	MENU_FUN_COMMON,//_MENU_FUN_ROTATE,	//旋转
	MENU_FUN_COMMON,//_MENU_FUN_OSD_ROTATE, //旋转
#endif
